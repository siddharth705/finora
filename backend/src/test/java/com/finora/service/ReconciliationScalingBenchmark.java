...
class ReconciliationScalingBenchmark {
    ...
    void measureReconcileAndDetectAcrossHistorySizes() {
        ...
        for (int size : SIZES) {
            List<Transaction> history = syntheticHistory(size);
            ...
            // magnitude. This is a rough warmup, not JMH -- good enough to answer "is this
            // seconds or milliseconds", which is the question the trigger actually asks.
            reconciliationService(history).reconcileForUser(userId);
            recurringService(history).detectForUser(userId);

            long reconcileMs = timeOf(() -> reconciliationService(history).reconcileForUser(userId));
            long detectMs = timeOf(() -> recurringService(history).detectForUser(userId));

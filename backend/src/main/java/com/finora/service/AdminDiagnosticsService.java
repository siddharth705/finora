...
public class AdminDiagnosticsService {
    ...
    private ApplicationInfoDto applicationInfo() {
        // Null when the app was started without the build-info Maven goal having run (e.g.
        // straight from an IDE run configuration, or `mvn spring-boot:run` -- that goal binds to
        // `package`, not `spring-boot:run`'s own lifecycle) -- see this class's own doc and
        // pom.xml's comments on that plugin. The git commit's own null case is BuildVersionResolver's
        // concern now (extracted so ImportSessionService can reuse the same answer).

...
|---|---|---|
| Credit card statement entity | `credit_card_statement` table, populated at confirm time from the fields already extracted (period, previous balance, purchases, cash advances, fees, payments/credits, total due, due date) — **not** minimum due, which nothing extracts today. Requires threading the staging-time evidence through to confirm (a staging→confirm contract change, not just a new table) — see [credit-card-statement-entity-design.md](credit-card-statement-entity-design.md). Visible to the user immediately. Payment-matching stays in Phase 3, since that part genuinely needs the graph. | 20% |
...
| Reversal split from refund | New enum value `REVERSAL`, split off the current shared "reversal" keyword — smallest possible fix for the biggest labeled gap in the audit. | 15% |

### Phase 2 — Transaction graph & confidence engine (1–2 months, foundation, not a user-facing feature)

| Item | Description | Weight |
|---|---|---|
| transaction_relationships table + canonical transaction layer | Ships with all ten relationship types *defined* in the enum (free — it's a schema value), but real detection logic is built for only four: `TRANSFER`, `REFUND`, `REVERSAL`, `DUPLICATE` — the ones that already exist today and just need an edge alongside their legacy column. `CC_PAYMENT` follows in Phase 3; `EMI`/`SALARY`/`LOAN_REPAYMENT`/`INVESTMENT_TRANSFER`/`CASH_WITHDRAWAL`/`CASH_DEPOSIT` stay enum-only, no matching service, until Phase 4 gives a reason to build one. | 35% |

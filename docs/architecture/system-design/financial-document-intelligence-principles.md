...
|---|---|---|---|
| ~~`ACCOUNT_NUMBER_RECOGNITION`~~ **Partially built (Phase 1C/1C.1)** | 6 of 7 real statements in the Aug 2026 validation pass | ~~High~~ **3 of 6 resolved; 3 deferred** | Original entry: `ACCOUNT_NUMBER`/`ACCOUNT_NUMBER_TRAILING_LABEL` only matched an explicit "Account Number" label; real statements embed it under an unrelated card-issuer label, or as a masked card number never labeled "Account Number" at all. **Resolved**: HDFC and Kotak (Phase 1C — `CARD_NUMBER_LABEL` vocabulary: Card Number/Card No./Credit Card No./Primary Card Number, same-line and trailing-label matching, tolerant of leading/trailing text) and SBI (Phase 1C.1 — a multi-line grid fallback, gated to fire only when the label is the last content on its own dedicated line, closing 3 false positives this same mechanism introduced on other documents before that gate was added). **Deferred**: HSBC and ICICI credit-card statements have no label anywhere near the masked value at all — recognizing them would require a shape-only, document-wide masked-number detector, rejected as unsafe (it would remove the label-proximity anchor every other case here relies on; see the Axis entry immediately below for the same reasoning applied to a different failure shape). Axis credit.pdf — see its own entry below, a distinct failure shape from HSBC/ICICI's. |
| Axis credit.pdf masked card number — label/value relationship destroyed by interleaved extraction artifacts | 1 statement (Axis credit.pdf) | Low — deferred, not a bug | The masked value genuinely exists in `section.auxiliaryText()`, inside `PdfMetadataExtractor`'s normal scope — this is not a `PdfTableLocator`/transaction-boundary problem. But it sits on a line where several unrelated Payment Summary grid columns were concatenated by extraction position (dates, amounts, an unrelated "register for Auto-Debit facility" enrollment sentence, then the masked number, then more amounts), while the document's own genuine "Credit Card Number" label sits on the adjacent line as a bare, disconnected grid header with no value of its own. Unlike SBI's grid case (label and value are the ONLY content on two structurally adjacent lines — a real document-authored relationship), Axis's label and value are only coincidentally near each other; the nearby text that could anchor a pattern ("Auto-Debit," "hassle free") is Axis-specific marketing boilerplate, confirmed unique to this one document, not a generalizable label. Building a rule keyed to it would be "when this particular Axis sentence appears near a masked number, assume that number is the card number" — exactly the document-specific-heuristic risk this project has deliberately avoided throughout Phase 1C. No safe generalized recovery rule identified from one document's evidence; revisit only if a second real document demonstrates the same structural pattern (label and value separated by interleaved, unrelated extraction content). The same masked value separately leaks into `section.rows()` twice on this document (a clean-looking transaction-table cell, and a severely corrupted 1223-character merged cell) — a distinct, pre-existing row-classification/table-formation defect, unrelated to this metadata gap since `PdfMetadataExtractor` never reads `rows()` at all; noted here for visibility, not part of this entry's scope. |
| `STATEMENT_DATE` — investigated, intentionally deferred | 5 of 6 real credit-card statements (AU, Axis, HDFC, ICICI, Kotak, SBI corpus; HSBC CC has no extractable label) print a distinct "Statement Date"/"Statement Generation Date" label; 3 of 9 real savings statements checked | Low — deferred, not missing evidence | The field genuinely exists and would cost about as much as Phase 1A's due-date fix (same `labelPattern`/`GRID_*_LABEL` + `findGridValue` machinery, no new parsing primitive) — but on every real credit-card document checked, the statement date is exactly equal to `statementPeriodEnd`, which the pipeline already extracts and surfaces; adding a separate field would mostly duplicate information already on screen. The two savings-account documents where it differs (by at most one day, plus a sub-day timestamp) aren't a case anything downstream currently uses. Not an evidence gap the way Axis's card number is — the evidence says this field is real but low-value as scoped. Revisit only if a genuine product requirement needs statement-generation-timestamp precision (e.g. reconciliation, statement versioning, disambiguating multiple same-day statements) — at that point the right concept is likely a distinct `statementDateTime`, not a `statementDate` that mostly restates `statementPeriodEnd`. |

...
| 2026-08-21 | **PR4-A merged (Entitlement architecture) — PR #211, squash `d0690a1c`. First of D-28's three sub-PRs.** Built the foundational piece PR4-B/PR4-C depend on: `Plan`/`Subscription`/`SubscriptionEvent`/`PlanChange`/`FeatureEntitlement` schema (V99, 3 seeded plans + a Free backfill for every existing user), fail-closed `EntitlementService.hasEntitlement` (deliberately the opposite default of the existing `FeatureFlag` system, which is fail-open by design), admin-only manual `SubscriptionService.changePlan` (upgrade/downgrade timing and refund policy are still open per D-28, so this always takes effect immediately -- the simplest behavior the schema supports, not a presumption about the eventual self-service flow), a `PremiumFeatureGate` frontend component, and a new admin Subscriptions page. Every new user auto-provisioned onto Free via `AuthService`. Self-review pass after the initial build (per this plan's standing "self-review every diff" rule) found and fixed three real gaps before merge: an audit actor-attribution bug (`changePlan`'s audit write had the record's subject/actor backwards relative to `AccountService.create()`'s established convention -- caught by the `AuditActorAttributionTest` ArchUnit gate), `AccountPurgeSweepService` never purging the new subscription tables on self-service account deletion (found proactively, not by CI -- fixed with `ON DELETE CASCADE` on the child tables + a new `SubscriptionRepository.hardDeleteByUserId`, proven by a real Postgres IT test that the cascade actually fires), and `SUBSCRIPTION_MANAGEMENT_VIEW` missing from admin-portal's central `ADMIN_PORTAL_PERMISSIONS` registry (would have locked an admin role holding only that permission out of login entirely -- caught by CI, same bug class this registry's own comment already documents once before). Backend suite 3219/3219, admin-portal suite 317/317, all 7 CI checks green on the final commit. PR4-B (billing history scaffolding) and PR4-C (referral program) remain, per D-28's sequencing, not yet started | Direct execution of D-28's own decision (first of its three named sub-PRs), logged per §12's "every merge gets a changelog row" practice; each of the three gaps gets its own mention per this plan's standing rule that a real bug found and fixed pre-merge is worth recording, not silently fixed |

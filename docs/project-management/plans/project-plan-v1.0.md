...
| | |
|---|---|
| **Overall completion toward v1.0 GA** | **83%, recomputed 2026-08-18 — see §12's 2026-08-18 entry for the full row-by-row reasoning.** First genuine recompute in three consecutive reports (held at 81% on 08-15 and 08-16, both explicitly deferring). Was 81% on 08-14–08-16, (stated as) 83% on 08-11–08-14, 82% on 08-10, 65% at 00:42 on 08-09 |
| **Current phase** | Phase 4 effectively complete (BH-042/043 fully closed, BH-045 deliberately descoped, BH-044 confirmed built and closed 2026-08-16 — see §4/§12); Phase 5 (production readiness) **not started**, though R-18 (Railway) closed this pass |
| **Health** | **On Track on engineering, stalled on everything that actually sets the date** — see §8 and §12's 2026-08-18 entry |
| **v1.0 scope** | Web + admin portal + **mobile** (D-2, 2026-08-09) |
| **Open bug-hunt findings** | **0 Critical, 0 High from the numbered backlog.** BH-042/043/044 fully closed; BH-045 deliberately descoped; **BH-060 independently re-verified closed 2026-08-18** (re-ran its regression tests directly against current `main`, not just re-quoted — 64/64 green). **PR #149 merged 2026-08-18** — the eager `fileContent` loading fix across 5 services is now on `main` (`8955e33e`). **Real remaining gaps, all unticketed:** R-19 (`DashboardService`/`InsightsService` date-anchoring bugs, confirmed still live 2026-08-18 — see §7), the Bank-entity audit-lookup gap from BH-044's re-verification |
| **Baselined against** | `origin/main` @ `fef2762d` (re-baselined 2026-08-18; previous full baseline `a2f2cad`/2026-08-16, 86 commits behind — see §12). **CI not confirmed green at the actual current tip** — rapid overlapping pushes are cancelling each other's runs via the branch's own concurrency group; the last commit anywhere in the chain with a fully completed green run is `b545cd20`, five commits behind. Every individual PR passed its own full CI on its own branch before merging (a code-quality fact, not in question); what's genuinely unconfirmed is a single green run at the literal current HEAD. **`MerchantLearningQueueIT`'s post-merge failure on PR #176 triaged 2026-08-18: re-run in isolation 3/3 times, 9/9 green each time** — consistent with the documented shared-context test-flake class (BH-058), not a code regression. **PR #149 merged same pass** (`8955e33e`), moving `main`'s tip past this report's own baseline |
| **Commits** | 836+ across 18 days (first commit 2026-07-31) |
| **Backend** | ~2985 tests at last individually-passing PR run (PR #176's own branch CI, pre-merge) — **not** re-confirmed as a single green number at the current tip, per the CI note above |
| **Clients** | Not re-counted this pass — see the 2026-08-16 entry's own count (36/40/30/12 files) as the last basis; a fresh count wasn't part of this session's scope |
| **CI** | Self-hosted macOS runner, smoke E2E blocking on every PR. **Currently showing a rapid-push cancellation pattern (see above), not confirmed green at HEAD** — a materially different, more honest statement than "green on `main`" |
| **Deployed** | Yes — `app.finoratech.info` (200 OK) + Railway backend (`UP`) + Railway Postgres. **R-18 closed 2026-08-18**: the current tip's Production deployment independently confirmed `state: success` via the GitHub Deployments API — the 08-16 "Deployment failed" status has fully resolved, production is serving the actual current build |
| **Store enrolment** | **Unchanged since 08-10, confirmed unchanged through 08-18 — zero evidence of movement found this pass either.** Apple: submitted as Individual, still not confirmed complete. Google Play: still not started. Still the single most stagnant item on the entire plan, now stagnant for 8 days |
| **Production-readiness audit** | **Complete** (2026-08-11), **its scope aging further** — since then: Gmail C6.1/C6.2, the account-lifecycle trilogy, Transaction Explanation, Design System, Real Dashboard Intelligence (PR3), Google Sign-In web (with its own real security fixes), Google Sign-In account-reauth (PR #175) have all shipped with no equivalent standalone security/architecture audit of their own — each carries its own PR-level review evidence (documented in §12's changelog), but none has had the dedicated pass the 08-11 audit gave the rest of the codebase |
...
|---|---|---|---|---|---|
| 1 | Core product (auth, ledger, accounts, budgets, goals, dashboard, reports, admin portal) | 20% | **96%** ▲ | 19.2 | Design System, Real Dashboard Intelligence (PR3-A/B/C), Phase C data export, and the full account-lifecycle round-out (instant delete, Google reauth across all 4 sensitive flows) all shipped and verified since 08-16. Held back from higher by **R-19's exposure now being real, not theoretical** — PR3 just built live UI (health score, financial journey) directly over the two services R-19 says compute wrong numbers for sparse histories. A few unmigrated TanStack pages remain (unchanged) |
| 2 | Import pipeline (M1 reliability + M2 at-scale) | 20% | **82%** ▲ | 16.4 | A dense, well-verified run of real parser/financial-correctness fixes this window (composite-account merge-corruption risk, an alias-race that could silently poison an import, a net-worth-snapshot race, ICICI header recovery, payment-summary false-positive detection) plus C6.4 reconciliation shipped. `PdfTableLocator` (1,358 lines) and `imports/product/` still **never reviewed**. The new corpus-wide ground-truth runner (built this window) now gives a real number instead of a guess: **16 PASS / 2 REVIEW / 3 FAIL across all 21 real documents** — 3 known-failing real statements is the honest remainder, not a vague "unreviewed risk" anymore |
| 3 | **Financial correctness defects** | 10% | **91%** ▲ | 9.1 | All six original P0s CLOSED–VERIFIED. **BH-060 independently re-verified closed this pass** (re-ran its own regression tests directly, not re-quoted). Three new instances of this exact defect class found *and* fixed with real race-condition/mutation tests this window — evidence the detection process works. Held back from higher: **R-19 is a known, unticketed, unfixed instance of the identical defect class**, now more exposed (see row 1) |
| 4 | Security & privacy | 12% | **93%** ▲ | 11.16 | D-23 Phase 1 (web Google Sign-In) shipped with three self-review passes that closed a real OAuth pre-hijacking vulnerability (password registration never verified email ownership — an attacker could pre-register a victim's email and inherit it via Google's own auto-link). PR #175's 8-angle adversarial review of the account-lifecycle Google-reauth gaps fixed 7 real findings pre-merge. Remainder: malware scanning, secret manager, edge security headers all still absent — and the **same CSP `connect-src` gap shipped, broke production, and needed a follow-up fix twice in one week** (Firebase reCAPTCHA, then Google Identity Services), a process gap worth naming, not just a code one |
| 5 | Testing & QA readiness | 12% | **86%** ▲ | 10.32 | BH-008/009/010 upgraded to CLOSED–VERIFIED; ground-truth corpus runner built; several real test-flake fixes (Import.test.tsx ×3, Dashboard clock, `UserControllerIT` race). Held back from higher: **no single commit at the actual current tip carries a confirmed-green CI run** — rapid overlapping pushes are cancelling each other's runs, and a fresh, untriaged failure (`MerchantLearningQueueIT`) surfaced during this pass. Every PR passed its own branch CI before merging; what's missing is a green run at HEAD itself |
| 6 | Infrastructure & production readiness | 14% | **60%** ▲ | 8.4 | **R-18 closed this pass** — the 08-16 "Deployment failed" status independently confirmed resolved via the GitHub Deployments API; the current tip is live and healthy. Everything else on this row is unchanged: no load-test remediation started, no secret manager, V73/V74 never applied to a non-test database |
| 7 | Mobile app | 12% | **71%** ▲ | 8.52 | D-23 Phase 2 (mobile Google Sign-In) + D-26 (Apple Sign-In, iOS) are **code-complete with real self-review evidence** (backend 3000/3000, mobile 324/324, one real pre-commit finding fixed) — PR #173 open, explicitly blocked on the owner registering real OAuth clients in Google Cloud Console and Apple Developer Portal, not on engineering. Mobile account-reactivation flow (#174) merged. **Store enrolment — the actual gate — moved zero millimetres**: Apple still not confirmed, Google Play still not started, 8 days stagnant |
| | **Total** | **100%** | | **83.1 → 83%** | **Recomputed 2026-08-18** — first genuine recompute since the 08-14 arithmetic correction; the 08-15 and 08-16 reports both explicitly held at 81% rather than recompute. Every row above carries its own reasoning so this number can be checked against the evidence, not trusted on its own — see §12's 2026-08-18 changelog entry for the full research trail |
...
### A real, ready fix that sat unmerged — PR #149 (found and merged 2026-08-18)

Session memory had recorded the eager `StatementImport.fileContent` (BYTEA) loading bug as fully
closed across all six call sites, "committed `1f16d3b`." That was checked against `origin/main`
directly this pass (`git merge-base --is-ancestor 1f16d3b origin/main` → false) and found **wrong** —
the fix for the remaining five call sites (`AccountService`, `AnalyticsService`,
`CapabilityCoverageService`, `StatementImportService`, `WorkspaceDashboardService`) existed only on
the still-open, fully green, mergeable **PR #149**, which had sat idle 26+ hours with zero reviews —
the **third** time in four days a real, ready, zero-risk PR sat unmerged for no technical reason
(`#130`, `#97`, then `#149` — see §7 R-20). **Merged 2026-08-18 (`8955e33e`)** after the owner's
go-ahead on this report. Memory corrected to match.
...
| R-17 | The plan file itself has been silently reverted twice by the shared-directory parallel-session pattern | Medium | Realized twice | Hours of PM analysis lost from disk each time, recoverable only because this session's own transcript retained it | Commit this file after every substantive update rather than leaving it as uncommitted working-tree state — see the note at the top of this document. **The 4 load-bearing docs this row's own text names committed 2026-08-18** (`Finora-v1.0-Decisions.xlsx`, `capability-audit-2026-08-14.md`, `finora-personal-cfo-experience.md`, `premium-experience-roadmap-2026.md`) — this file itself already gets committed every pass, so the mitigation is now consistently applied to all of R-17's named load-bearing docs. **~20 other untracked files remain in the primary checkout** (various proposal/investigation docs) — deliberately left alone, out of R-17's named scope and possibly another session's in-progress work | 🟡 Mitigated for the named docs, other untracked files unreviewed |
...
| R-18 | ~~GitHub's deployment status on the current `main` tip reads "Deployment failed" (Railway)~~ | — | — | — | **Closed 2026-08-18** — independently re-checked via the GitHub Deployments API (`gh api .../deployments`, `.../statuses`): the current tip's Production deployment shows `state: success`; `app.finoratech.info` returns 200 and `api.finoratech.info/actuator/health` returns `UP`. Not inferred from the earlier "site responds normally" signal alone — the deployment record itself now confirms success | ✅ Closed |
| R-19 | Real correctness bugs found in `DashboardService`/`InsightsService`'s date-anchoring for sparse/gappy transaction histories, discovered as a side effect of BH-042's review | Unknown — not yet scoped | Confirmed to exist, per PR #127's own scope note; **confirmed still live 2026-08-18** — both services still call the same unbounded/simple-index logic at the lines originally flagged | Same defect class (BH-001/003/004/005/006) that produced this project's original P0 balance-corruption findings; currently has no bug-hunt ID and no owner. **Exposure raised, not lowered, this pass**: PR3-A/B/C just shipped the Financial Journey dashboard and health-score UI directly on top of the two affected services, so a sparse-history user is now more likely to see the wrong number on a screen, not just in an internal computation | Give it a real BH-0XX ID and scope it, rather than leaving it as an unlabeled PR footnote | 🔴 Open, **exposure increased** |
| R-20 | A fully green, zero-risk, mergeable PR sits unreviewed for a day or more before merging, for no technical reason | Low severity per-incident, **High as a pattern** | Observed 3 times in 4 days (#130, #97, #149 — #149 merged 2026-08-18 same pass it was found) | Real fixes (data export, regression-coverage upgrades, a 5-site eager-load memory bug) sat off `main` for no reason other than nobody clicked merge. The pattern itself — not any one instance — is the open risk: nothing structural stops a fourth one from sitting idle the same way | No process change proposed yet — naming the pattern is this pass's contribution; needs an owner decision on whether ready PRs should have a merge SLA | 🟡 Latest instance closed, pattern itself still open |
| R-21 | ~~A new, unexplained CI failure surfaced on `main` itself, untriaged~~ | — | — | — | **Triaged 2026-08-18: `MerchantLearningQueueIT` re-run in isolation 3 times, 9/9 tests green every time.** The test file itself hasn't changed since 2026-08-14 (PR #144). Consistent with a repeat of the already-documented shared-context test-isolation flake class (BH-058), not a real regression in the merchant-learning-queue recovery logic — this run's own premise-check failure (fixture found `PENDING` instead of the expected stranded `PROCESSING`) doesn't reproduce outside whatever full-suite ordering produced it on PR #176's run. Not proof of zero risk of recurrence — the same class of flake can resurface under the same shared-context conditions — but it is proof this isn't a logic bug introduced by recent changes | 🟢 Closed as flake, watch for recurrence |
...
| Date | Change | Why |
|---|---|---|
| 2026-08-18 | **Follow-up to this same day's PM status report: owner said "go ahead" on the report's own recommendations. Three actions executed.** (1) **PR #149 merged** (`8955e33e`) — real, green, zero-risk fix, no reason left to hold it; §4/§7 R-20/§1 updated to match. (2) **`MerchantLearningQueueIT` triaged**: re-run in isolation 3 times against the pre-merge commit, 9/9 green every run; the test file hasn't changed since 08-14 (PR #144). Consistent with the documented shared-context test-flake class (BH-058), not a logic regression — R-21 closed as flake, not as zero-risk-of-recurrence. (3) **R-17's 4 named load-bearing docs committed** (`Finora-v1.0-Decisions.xlsx`, `capability-audit-2026-08-14.md`, `finora-personal-cfo-experience.md`, `premium-experience-roadmap-2026.md`) — copied from the primary checkout's untracked working tree into this worktree and committed as-is. **Deliberately not touched**: ~20 other untracked files in the primary checkout (assorted proposal/investigation docs, `CLAUDE.md`) — outside what this report actually proposed committing, and possibly another session's in-progress work; flagging for a separate decision rather than bundling in | Owner authorized the report's own named next actions with "go ahead" rather than picking specific items — executed exactly the three items that were mine to do (merge, triage, commit the named docs), left the owner-only items (OAuth client registration, tester lineup, business decisions) untouched since those aren't mine to execute |
| 2026-08-18 | **Full PM status report, re-baselined against `origin/main` @ `fef2762d` (86 commits since the last full baseline `a2f2cad`, 2026-08-16, plus PR #171 merging mid-investigation). Headline moves from 81% to 83% — the first genuine recompute of §2 in three consecutive reports, not another hold.** Three parallel research passes (full plan re-read, targeted claim verification, categorized 86-commit survey) plus direct checks (fresh `gh`/`curl`/local-test evidence, not re-quoted from prior entries). **§2 recomputed row-by-row, shown with reasoning, not asserted:** Core product 95→96% (Design System, Real Dashboard Intelligence PR3-A/B/C, Phase C export, account-lifecycle rounding-out all shipped and verified, offset by R-19's now-heightened exposure — PR3 just built new UI directly over the affected DashboardService/InsightsService surfaces); Import pipeline 78→82% (a dense, well-verified run of real parser/financial-correctness fixes — composite-account merge-corruption risk, an alias-race that could poison an import, a net-worth snapshot race, ICICI header recovery, payment-summary false-positive tables — plus C6.4 reconciliation shipped and a corpus-wide ground-truth runner now showing 16 PASS/2 REVIEW/3 FAIL across all 21 real documents; `PdfTableLocator`/`imports/product/` still unreviewed); Financial correctness defects 90→91% (BH-060 independently re-verified closed — see below — and three new instances of the exact same defect class found+fixed with real race/mutation tests this window, offset by R-19 remaining a known, unticketed, unfixed instance of that same class); Security & privacy 90→93% (D-23 Phase 1 web Google Sign-In shipped with three self-review passes that closed a real OAuth pre-hijacking vulnerability — password registration never verified email ownership, so an attacker could pre-register a victim's email and silently inherit it via Google's auto-link — plus PR #175's 8-angle adversarial review of the account-lifecycle Google-reauth gaps, 7 real findings fixed pre-merge; offset by malware scanning/secret manager still absent and the *same* CSP connect-src gap shipping unnoticed twice in one week, a process gap worth naming); Testing & QA readiness 85→86% (BH-008/009/010 upgraded to CLOSED–VERIFIED, ground-truth runner built, several real test-flake fixes, offset by a fresh, unexplained CI failure and no single commit at the true current tip carrying a confirmed-green run — detail below); Infrastructure & production readiness 57→60% (R-18's Railway "Deployment failed" status fully resolved and independently confirmed — the current tip is live and healthy — nothing else on this row moved); Mobile 68→71% (D-23 Phase 2 + D-26 Apple Sign-In are code-complete with real self-review, PR #173 open — but store enrolment, the actual gate, moved zero millimetres). **New weighted total: 19.2+16.4+9.1+11.16+10.32+8.4+8.52 = 83.1% → 83%.** **What's new and verified, not previously reflected:** (1) **BH-060 independently re-verified**, not just re-quoted — re-ran `TransactionNormalizerTest`/`PdfPreviewGeneratorTest`/`PreviewGeneratorTest` directly against current `HEAD`: 64/64 green, `RowKind.BALANCE_MARKER` confirmed still wired through every call site grepped. (2) **R-18 (Railway deployment) closed** — `gh api .../deployments` shows the current tip's Production deployment `state: success`, `app.finoratech.info` returns 200, `api.finoratech.info/actuator/health` returns `UP`. (3) **R-19 confirmed still open, unticketed, unfixed** — `DashboardService`/`InsightsService` both still call the unbounded `findByUserId` at the exact lines flagged 08-16; `InsightsService`'s prior-month slicing is still simple list-index arithmetic, not the calendar-aware fix `DashboardService` already got for its own equivalent case (that fix pre-dates R-19's discovery and is unrelated to it — confirmed by reading the diff, not the commit message). PR3-A's minimum-data floor is a different, unrelated fix (a display-hiding threshold) that does not touch this defect. (4) **A real, verified discrepancy between session memory and `main`'s actual state**: memory recorded the eager `fileContent` BYTEA-load bug (PR #142 fixed one of six call sites) as fully closed across the other five, "committed `1f16d3b`." **That commit is not on `main`** — `git merge-base --is-ancestor 1f16d3b origin/main` returns false; it lives only on the still-open branch behind **PR #149**, which has been green (all 7 checks passing, `MERGEABLE`) and completely unreviewed for 26+ hours. `AccountService`, `AnalyticsService`, `CapabilityCoverageService`, `StatementImportService`, `WorkspaceDashboardService` all still call the entity-returning finder that eagerly loads up to 10 MB of statement bytes into heap per call — confirmed live on current `main` by direct grep, not inferred. Corrected in memory as part of this pass. (5) **The uncommitted load-bearing files flagged 08-16 are still uncommitted two days later** — `git log --all` shows zero history for any of the four across every local and remote ref; all four still sit, unchanged, only in the primary checkout's untracked working tree, fully exposed to the loss pattern R-17 already names. (6) **A new, unexplained CI failure surfaced on `main` during this investigation itself**: PR #176's post-merge push-triggered run failed `Backend (Java 25)` — `MerchantLearningQueueIT.anEventAbandonedInProcessingIsReturnedToTheQueue`'s own *premise*-check assertion (line 436, confirming the test's stranded fixture is still `PROCESSING` before recovery runs) found `PENDING` instead, meaning something reset the row before the test's own recovery call. Not yet independently re-run in isolation to tell a real regression from this repo's own documented class of shared-context test-isolation flake (BH-058) apart — flagged as needing triage, not diagnosed. **Also observed, not previously named:** `main` is currently taking rapid, closely-spaced pushes whose CI runs cancel each other via the branch's own concurrency group before finishing — the last commit anywhere in the chain with a fully completed green run is `b545cd20`, five commits behind the tip this report is baselined against. Every individual PR in that chain passed its own full CI on its own branch before merging (not a code-quality gap), but there is currently no single commit at `main`'s actual HEAD with a confirmed-green post-merge run — an honest gap, not a claim of "CI green on main" this pass. **§7 updated**: R-18 closed; R-19 left open with its exposure explicitly re-stated as higher, not lower, now that PR3 ships real UI on the affected surfaces; two new risks named for the idle-PR-#149 pattern and the untriaged CI failure. **§4 updated**: PR #149 logged as a real, ready, unmerged fix — the third time in four days a fully green, zero-risk PR has sat idle purely because nobody clicked merge (#130, #97, now #149) — worth naming as a process pattern, not three unrelated coincidences. **§9/§11 unchanged in substance**: D-7's own row still names actual price points, payment gateway integration, and a placeholder business address as the remaining blockers, none of them engineering — inventing a fresh Best/Target/Conservative date off that would be exactly the asserted-not-derived figure this file's own opening line disclaims, so the stale table stays stale. **D-23/D-26 status corrected**: Phase 2 (mobile Google + Apple Sign-In) is code-complete with real self-review evidence (backend 3000/3000, mobile 324/324, one real pre-commit finding fixed) but genuinely unmerged, blocked specifically on the owner registering real OAuth clients in Google Cloud Console and Apple Developer Portal — a materially different state than either "not started" or "shipped," and the file previously only supported those two labels | Requested as a full PM status update ("what's next?"). The point of actually doing the §2 recompute this time, after two consecutive reports that declined to, is that declining forever is its own kind of dishonesty once enough evidence has accumulated to do it properly — every row above is shown with its reasoning so the number can be checked, not trusted |

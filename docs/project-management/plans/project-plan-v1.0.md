...
| **D-23** | **Owner asked why "Sign in with Google" doesn't appear on the register/login screen (a screenshot). It doesn't exist — the only Google integration is Gmail Transaction Sync, a Settings-page feature for an already-logged-in user, unrelated to authentication. Add real Google sign-in to registration and login, "same as mobile app" — a genuinely new, security-sensitive feature, not a bug fix.** | — | ✅ **Decided 2026-08-16, phased.** Investigated before designing, same discipline as every other D-row: `AuthController.java` already carries a `// TODO Phase 2: /oauth/google callback endpoint` placeholder, and `GoogleOAuthProperties`'s own doc comment states outright "this is not Sign in with Google" — anticipated, never built. Three real security/product forks surfaced and put to the owner rather than assumed: **(1) account linking** when a Google email matches an existing password account — **owner chose auto-link on Google's verified email** (the same trust basis "Sign in with Google" uses everywhere), not reject-and-redirect. **(2) phone verification** for Google signups, who arrive with no phone number at all — **owner chose to keep it mandatory for everyone**, routing a new Google-created account through the same OTP step every other signup already goes through, rather than carving out a permanent gap in the one security control that currently covers every account uniformly. **(3) sign-in flow** — **owner chose Google Identity Services** (a real Google-rendered button, ID token posted to a new backend endpoint, verified via `GoogleIdTokenVerifier`) over a popup+authorization-code flow matching Gmail sync's own pattern; owner already has a Google Cloud OAuth client for this, separate from the Gmail-sync one. **Phased: Phase 1 (web) first, Phase 2 (mobile) reusing the same backend endpoint** — mobile's Expo config-plugin + dev-client workflow (already proven with `@react-native-firebase`) fits `@react-native-google-signin/google-signin` cleanly, no prebuild/eject prerequisite; the backend ID-token verification is identical for both platforms, only the client-side SDK differs. **Implementation notes found during scoping, not owner decisions**: no schema migration needed (`phone_number` is already nullable; a new Google-only account gets a random, cryptographically unguessable `password_hash` rather than a nullable column); the existing `PhoneChangeService`/`phone-change/start\|verify-otp\|complete` flow (already reachable by a phone-unverified user per `PhoneVerificationFilter`'s own allowlist) is reused as-is to collect a Google signup's first phone number, rather than building a new screen. |
| ~~**D-24**~~ | ~~D-22's PR2 (Design System) scoped: which of the seven proposed components (`FinoraCard`, `MetricCard`, `EmptyState`, `SectionHeader`, `QuickActionCard`, `ChartContainer`, `PremiumBadge`) have real drift to fix vs. are pure prep-work, and how far should PR2's migration go -- components only, components + the one live small bug, or a full page migration?~~** | — | ✅ **Decided 2026-08-16: (c), full page migration.** Audited every app-shell page against each of the seven proposed components before answering, per this plan's own §8a rule -- not assumed from the pitch. Full findings: [`design-system-proposal.md`](../../proposals/design-system-proposal.md). Six of seven have real, counted drift: `FinoraCard` (43 raw duplications of the same card shell across `pages/`/`components/`), `MetricCard` (Dashboard's KPI cards and `Reports.tsx`'s Income/Expense/Net trio render at two different type scales, one of them using a raw `text-gray-500` instead of the `muted` token), `EmptyState` (Dashboard's new rich pattern vs. 8 bare one-line empty-text sites across 6 other files, using **three different color tokens** for the same meaning -- `Goals.tsx`'s `text-gray-300` is close to invisible and may be a live bug, not just inconsistency), `SectionHeader` (the heading+"View All" pair is copy-pasted 5 times inside `Dashboard.tsx` alone), `ChartContainer` (Dashboard and `Investments.tsx` each hand-build an independent Chart.js `options` object; `Reports.tsx` renders its own distribution chart a third, unrelated way). Two are pure prep with nothing to migrate: `QuickActionCard` (Dashboard's 7-item grid is the only action-tile grid in the app) and `PremiumBadge` (Dashboard's one "Beta" pill is the only badge in the app; no tier badges exist anywhere since D-7 pricing is still open). A separate, larger number -- **99 raw `text-gray-*` usages** app-wide -- is flagged as real but deliberately *not* proposed as part of PR2 (its own mechanical sweep, not bundled in). **Owner chose (c), full page migration** -- the most thorough of the three options, not the "narrowest slice first" default this plan's other D-rows have leaned toward. Scope: all 7 components built, Dashboard's own usages migrated onto them, the 6-file/8-site `EmptyState` gap closed (including the near-invisible `Goals.tsx` bug), and `Reports.tsx`/`Investments.tsx` brought onto `MetricCard`/`ChartContainer` too. Files touched: `Dashboard.tsx`, `Reports.tsx`, `Investments.tsx`, `Budgets.tsx`, `Goals.tsx`, `StatementHistory.tsx`, `Insights.tsx`, plus the 7 new `design-system/` files. **Still explicitly out of scope**, unchanged by this choice: the 99-site raw-`text-gray-*` sweep and the marketing/auth pages -- neither was ever one of the three options. |
| ~~**D-25**~~ | ~~D-22's PR3 (Real Dashboard Intelligence) scoped: of the three pieces D-22 named (real cash-flow timeline, Financial Health Score minimum-data floor, "Financial Journey" milestone section), which are real work vs. already done, and what are the exact thresholds/milestones?~~ | — | ✅ **Decided 2026-08-17.** Audited all three against current code before answering, per §8a. Full findings: [`dashboard-intelligence-pr3-proposal.md`](../../proposals/dashboard-intelligence-pr3-proposal.md). **(1) Real cash-flow timeline — already done, drop from PR3.** `Dashboard.tsx`'s Cash Flow Overview already renders real per-month income/expense data with a 3/6/12-month range selector (`reportsApi.forMonth`); `git log -S` confirms this predates D-22 even naming it -- D-22's own framing was stale the moment it was written, same "audit found it already exists" pattern as D-19/D-20. **(2) Health Score minimum-data floor — real, confirmed.** Traced `DashboardService.computeHealthScore` component-by-component: a thin-data user (few transactions, one month of history) can genuinely land under 40 ("Needs Attention") by construction, not hypothetically -- `cashFlowScore` can hit 0% if that one month's expenses exceed income, `emergencyScore` near 0 if tracked liquid savings are small. Confirms D-22's own finding was accurate, not overstated. A true zero-transaction user already never sees this (D-19's existing `isEmpty` gate) -- the gap is specifically the thin-but-not-zero user. **Owner chose: transaction-count floor** (e.g. ≥10 transactions) **and a "Getting Started / X% complete" progress card below it** (D-22's own mockup), rather than time-span or simply extending the existing hide-the-section gate. **(3) "Financial Journey" — genuinely new, confirmed.** Grepped for any existing milestone/journey/onboarding-progress infrastructure app-wide -- the only hit is the **marketing landing page's** static narrative section, unreachable from the authenticated app, not a per-user tracked feature. Raw timestamp data for 3 of 4 candidate milestones already exists unaggregated (`User.createdAt`, `StatementImport.importedAt`, `Budget.createdAt`/`Goal.createdAt` via `BaseEntity`); the 4th (goal *completion*, not creation) needs a new `completedAt` column since only `updatedAt` exists today and any unrelated edit would also bump it. **Owner chose all 5 milestones, including goal-achieved** -- asked to clarify an initially-ambiguous answer ("create backend") against the doc's own 4-vs-5 framing before recording this, rather than guessing; confirmed explicitly. Scope now includes the new `completedAt` tracking the 4 free milestones didn't need. **Owner chose event-based framing over the original pitch's fixed Day 1/3/15/30 schedule** -- each milestone shows its own real elapsed-days-since-signup once it actually happens, not a prescribed timeline. |

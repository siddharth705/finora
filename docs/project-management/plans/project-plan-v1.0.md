...
| | |
|---|---|
| **Overall completion toward v1.0 GA** | **81% → 83% — see §2's recomputed table.** Real progress across five of seven workstreams (Google/Apple sign-in shipped both platforms, PR3 A–D dashboard intelligence complete, security-review Sprints 1/2/4 merged, mobile hardening + rebrand shipped, R-18's deployment failure resolved), partially offset by a second, independent full-repository bug review (§4c, new) landing 46 live findings — including live Highs — that were not previously counted anywhere in this plan. Net is still forward: closing an unreviewed-surface gap by finding and documenting it is itself progress, per this plan's own §12 discipline, even before the findings are fixed |
| **Current phase** | Phase 4 (original bug hunt) complete. **Phase 4c (full-repo bug review, new) open — 46 live findings, remediation not started as engineering scoped to this plan.** Phase 5 (production readiness) **not started**. **Phase 2 (Transaction Boundary & Extraction Reliability)** is in progress as the parser-reliability track after §4a's metadata work — 2A–2D and 2E.1–2E.2 complete and merged (6 PRs), 2E.3+ and 2F–2H not started — see §4d and D-29; not a v1.0 gate, same as §4a |
...
| **Open bug-hunt findings** | **Original numbered backlog (BH-0XX): 0 Critical, 0 High still open** — unchanged, all P0/P1/P2 items from §4 remain closed or deliberately deferred. **New, separate: the §4c full-repo review found 46 live findings (3–4 Live High, ~18 Live Medium, ~24 Live Low), on top of 3 Criticals and up to 8 Highs found-and-fixed during the same review** — full detail in [`BUG_REVIEW_REPORT.md`](../../quality/bug-reports/BUG_REVIEW_REPORT.md). **BH-061 remains open** (81% of sections / 100% of credit-card sections never get a usable account/card number — root cause in `PdfMetadataExtractor`) — Phase 1C/1C.1 (§4a) fixed the HDFC/Kotak/SBI slice of this exact root cause since BH-061 was filed, but the baseline has not been re-measured against the corpus, so BH-061 stays open rather than being asserted closed |
...
|---|---|---|---|---|---|
| 1 | Core product (auth, ledger, accounts, budgets, goals, dashboard, reports, admin portal) | 20% | **94%** ▼ | 18.8 | Real shipped progress (Sign-in with Google/Apple both platforms, instant account deletion, PR3 A–D dashboard intelligence) offset by §4c's review surfacing live gaps in already-counted surface: Bug 10 (`GET /recurring` mutates), Bug 29 (Swagger UI unauthenticated off-`prod`), Bug 50 (`CurrentUser.id()` 500s instead of 401 on anonymous), Bug 53 (refresh cookie inert cross-domain), Bug 57 (`ROLE_null` grant) |
| 2 | Import pipeline (M1 reliability + M2 at-scale) | 20% | **80%** ▲ | 16.0 | Phase 1 metadata reliability closed (Payment Due Date, Total Amount Due, masked card number HDFC/Kotak/SBI — §4a), evidence-based import reliability status shipped (#184). Offsetting: BH-061 (new, open — 81%/100% of CC sections lack a usable card number), plus §4c's Bug 04 (LIVE High — unbounded category name rolls back an entire import), Bug 30/33/34/56. `PdfTableLocator`/`imports/product/` still never fully reviewed |
| 3 | **Financial correctness defects** | 10% | **88%** ▼ | 8.8 | §4c's review found and **fixed** 3 new Criticals (merchant-learning queue unreachable, unvalidated balance write, UPI/NEFT merchant collapse) plus 5 Highs — real, substantial closure. Offsetting: Bug 04 (LIVE High, import rollback) and Bug 35 (LIVE Medium — `BudgetService.upsert` always reports spend as zero) are newly-known live defects, not previously ticketed anywhere |
| 4 | Security & privacy | 12% | **92%** ▲ | 11.0 | Security-review Sprints 1/2/4 merged (#186/#187/#189 — SEC-06/13/02(partial)/01/07/03(backend, flag-gated)/09/08/17). Sprint 3 (SEC-05/10/11/12/16) **built, PR #188 open, not yet merged**. §4c also surfaced security-adjacent live findings outside the original 18-item backlog: Bug 07 (429 responses carry no CORS headers), Bug 26 (`PhoneVerificationFilter` fails open) |
| 5 | Testing & QA readiness | 12% | **90%** ▲ | 10.8 | Suite now 3200 backend / 569 frontend / 314 admin / 364 mobile, all green — real counts, not file counts, for the first time. The §4c full-repo review (58 findings, re-validated against a moving tree) is itself a QA-readiness deliverable |
| 6 | Infrastructure & production readiness | 14% | **60%** ▲ | 8.4 | **First real movement since 08-10**: R-18 (deployment failure on the latest tip) resolved — GitHub's deployment API confirms the current `main` tip deployed successfully. Everything else genuinely unchanged: no restore drill, no load test since 08-16's D-4 experiment, no secret manager |
| 7 | Mobile app | 12% | **76%** ▲ | 9.1 | Sign-in with Google/Apple shipped (D-23 Phase 2, D-26), Sprint 4 mobile hardening merged (biometric lock, root detection, screenshot guard), rebrand to Quiet Mark shipped, bundle identifiers renamed to `com.finoratech.app`. Store enrolment itself unchanged (§1) and still no confirmed physical-device run |
| | **Total** | **100%** | | **82.9 → 83%** | **Re-baselined 2026-08-21.** Net +2 points on real shipped progress across 5 of 7 rows, partly offset by §4c's newly-discovered live findings in rows 1–3. This is a genuine recompute, not a held figure — every row above cites the PR or commit behind its delta |
...
**Sprint 3 — Security maturity. Built, PR [#188](https://github.com/siddharth705/finora/pull/188) open since 2026-08-18, not yet merged as of this 2026-08-21 re-baseline** (per PR #188's title, covers SEC-05/10/11/12/16)
...
Medium, 4 live), 20 Medium (2 fixed, 18 live), 25 Low (1 fixed, 24 live). 46 total live, 11 fixed.**
...
| 2G | Transaction reconciliation — extracted transactions vs. statement totals | ✅ **Fixed.** New `BalanceSequenceResolver` (anchor-propagation from the statement's own opening balance, never the printed closing balance) replaces `BalanceChainUtil`'s silent same-day mis-ordering fallback; both call sites (CSV `StatementValidator`, PDF `PdfPreviewGenerator`) migrated together. `balance-chain-ordering-design.md`, PR #259 (2026-08-22). Post-merge real-corpus audit (§7.2 of the design doc, PR #263) confirms zero `STATEMENT_TOTALS` failures/warnings across all 25 real documents |
...
| 2E.3 | Narration correctness (owner's original framing: "ICICI savings description is wrong for 11 rows") | ✅ **Closed — not reproduced.** Ran the real ICICI savings statement through the diagnostic pipeline: all 11 transactions extract with well-formed, correctly-separated narration on current `main`; `COLUMN_AMBIGUITY`/`ROW_ACCOUNTING` both `VERIFIED`. The claim predates Phase 2 and was carried forward without ever being checked. Investigating it surfaced a real, unrelated defect instead — the closing-balance bug that became 2G. `same-day-reversal-closing-balance-investigation.md`, PR #250 |
| 2E.4 | Balance semantics (owner's original framing: "CBI opening balance detected as 47.77") | ✅ **Closed — not reproduced.** Traced the real CBI statement's own first-transaction balance (an Indian comma-grouped shape matching the claim's structure) through every parsing stage in the PDF import path: `CsvParser.parseNumeric` strips commas correctly regardless of grouping position, and the one live path that could set `openingBalance` for this document (`BalanceSequenceResolver`) uses the same correct parser. The field is `null` for this document by Phase 2G's deliberate ambiguity-suppression design, not a wrong value. `cbi-opening-balance-investigation.md` |
...
**Status: 2A–2D complete and merged. 2E.1–2E.2 complete and merged. 2E.3–2E.4 closed (not reproduced). 2G complete and merged. 2E.5, 2F, 2H not started.**
...
| R-20 | §4c's full-repository bug review found 46 live findings (3–4 Live High) with no owner sequencing decision, four days after landing on `main` unreferenced by this plan | Medium, pending owner sequencing | Confirmed, `main` @ `a73bee21` | Same class of risk §4b's 18-item backlog already carries, but larger (46 vs 18) and entirely unsequenced. Bug 04 specifically (unbounded category name rolls back an entire import) overlaps financial-correctness territory (§2 row 3) | Pose the sequencing question to the owner the way §4b's 4-sprint order was posed, rather than letting it sit undecided | 🔴 Open, new |
...
| **QA Complete** | 🔴 | **Updated 2026-08-21 — two backlogs now, not one.** Original numbered bug-hunt backlog (§4): 0 Critical, 0 High, unchanged (P0–P3 all closed/accepted except BH-044's engineering, not yet built, and BH-045, deliberately descoped). **New: §4c's independent full-repo review found 3–4 Live High findings** (Bug 04/07/10, see §4c) that criterion 1 below ("zero open Critical or High defects") does not currently exclude just because they came from a different review — until the owner explicitly scopes criterion 1 to one backlog or the other, read it as failing on §4c's Live Highs alone. What else remains: full E2E still not in CI; cross-browser still never green; the 7 named test gaps below are unverified this pass (carried forward, not re-checked) |
...
| 2026-08-22 | **2G closed out of sequence, ahead of 2E.3–2E.5/2F (§4d touched, §2 not recomputed, same Phase-2-outside-the-weighted-score standing).** While verifying an owner claim about 2E.3 (ICICI savings narration), found a real, independent bug in `BalanceChainUtil.first()/last()`: it silently mis-orders a same-day transaction cluster whenever a subset nets to zero, producing a confidently wrong `closingBalance` that `StatementTotalsValidator` then misdiagnosed as bad transaction rows rather than a bad derived balance. Five design decisions (owner-approved), a 25-document/29-section real-corpus sweep confirming the anchor-propagation design resolves 100% of real evidence and surfacing a second independent live instance of the bug, then `BalanceSequenceResolver` implementation — `balance-chain-ordering-design.md`, PR #259, merged with two live-bug documents (ICICI savings, the second one the sweep found) re-verified `VERIFIED` against the real files. A follow-up post-merge audit (owner's own ask: "any `STATEMENT_TOTALS` failures left now that ordering doesn't contaminate results?") re-swept the same 25-document corpus and found **zero** — written up as design-doc §7.2, PR #263 | Direct record of an out-of-sequence but fully investigate-then-design-then-implement-then-verify phase closure, found while checking an unrelated owner claim, same "close the reference gap the moment it's found" discipline as the entry below |

...
| **Current phase** | Phase 4 (original bug hunt) complete. **Phase 4c (full-repo bug review, new) open — 46 live findings, remediation not started as engineering scoped to this plan.** Phase 5 (production readiness) **not started**. A **Phase 2 (Transaction Boundary & Extraction Reliability)** has also been scoped as the next parser-reliability phase after §4a's metadata work — see new §4d and D-29; not a v1.0 gate, same as §4a |
...
| — | **Phase 2 — Transaction Boundary & Extraction Reliability (new, 2026-08-21, D-29)** | ⬜ **Not started — 2A investigation next** | Follows §4a's metadata-reliability work (Phase 1). Scoped 2A–2H (start/end detection, non-transaction noise, row completeness/correctness, multi-page continuation, reconciliation, import-result reporting). Investigation-first, same discipline as Phase 1 — see new §4d and D-29 |
...
|---|---|---|
| 2A | Transaction start — identify where real transactions begin | Partially working |
| 2B | Transaction end — stop at the last real transaction | 🔴 Known architectural gap — the main extraction path can keep a section open past a closing marker, so later content with a date and amount can be mistaken for another transaction |
| 2C | Non-transaction content — prevent ads/rewards/T&Cs/illustrations from becoming transactions | 🔴 Evidence exists, not fully solved |
| 2D | Row completeness — every real transaction becomes a staged row | Needs corpus validation |
| 2E | Row correctness — date/description/debit/credit/balance belong to the same transaction | Needs corpus validation |
| 2F | Multi-page statements — correctly continue/terminate the transaction table across pages | Needs corpus validation |
| 2G | Transaction reconciliation — extracted transactions vs. statement totals | Existing validators (`BalanceChainValidator`, `StatementTotalsValidator`) can likely become the evidence layer, not built from scratch |
| 2H | Import result — a trustworthy "what was imported and what wasn't" explanation to the user | `ImportReliabilityStatus` (§4a's evidence-based status, PR #184) is a foundation to extend, not a new mechanism |

**Sequencing: 2A is an investigation, not an implementation, same discipline as Phase 1.** Phase 1
found twice that an obvious theory about a defect's cause was wrong before the real cause surfaced
(the Payment Due Date ordinal-suffix fix, the Axis card-number root-cause correction) — 2B
specifically (transaction-end detection) is exactly the kind of defect a keyword-list fix would
appear to solve while missing the structural cause. **Do not change row grouping
(`groupIntoRows`/`PdfTableLocator`) until real-corpus evidence proves where the actual signal is** —
closing headers, repeated table headers, page transitions, column geometry, or something else
entirely. Build synthetic fixtures only after a real-corpus failure shape is confirmed, same as every
Phase 1 fix.

**Status: not started. Phase 2A (Transaction Boundary Investigation) is the next task.**

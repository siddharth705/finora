...
| **4** | **Hardening & Defect Remediation** | 🟡 **~99% — BH-044 engineering built 08-15, PR pending** | Corrected 2026-08-14, updated 08-15 — all P0–P3 findings closed, accepted, or decided (§4). `BH-044`'s retention direction (redact) decided 08-15 and the redaction sweep itself built same day (branch `fix/bh-044-audit-log-redaction`, not yet merged) — see §4's BH-044 entry for the migration-deadlock catch and BH-044.1, opened and deliberately deferred with owner sign-off. `BH-042`/`043`/`045` owned by a parallel session, in progress elsewhere. Not marked ✅ Complete because BH-044's PR hasn't merged yet and `BH-042`/`043`/`045` remain open |
...
| — | Gmail Sync (C1–C5.3: OAuth, sender trust gate, discovery, 6 merchant parsers) | ✅ Backend complete, 🟡 **C5.4 (user-facing completion) next, scoped 2026-08-15 — see D-15** | PRs #104–#119, all merged. Same "grew outside the phase table" pattern §12 already flags for other unweighted work — not yet in §1/§2, security review still owed (see 08-15 changelog) |
...
- **`BH-044`** (`audit_logs` grows unbounded, and a `RECONCILIATION_RUN` row per write was a large
  ...
  defaults: metadata retained long-term, financial payload redacted after 730 days. **Engineering
  built 2026-08-15** (branch `fix/bh-044-audit-log-redaction`): `AuditService.redactExpiredMetadata()`,
  a scheduled sweep following the existing `StatementStorageSweepService` pattern. In-place
  redaction on the existing table, not a split into a separate `audit_payloads` table — the split
  wasn't needed to satisfy the owner's actual decision and would have added schema-migration risk
  for no gain toward it. A 7-agent review before it shipped found and fixed a real migration
  deadlock (`CREATE INDEX CONCURRENTLY` hung application startup indefinitely against this
  environment's connection-pool behavior — reverted to a plain, transactional index) plus a test-
  isolation bug and two smaller fault-tolerance gaps; full backend suite 331/331.
  **BH-044.1 opened, not started:** review also found that redacting a Bank audit row's metadata
  permanently breaks the admin Bank audit tab (`AuditLogRepository.findByBankIdInMetadata`) after
  the retention window, since `Bank` entities record `entityId = null` and their only identity
  lives inside the metadata this sweep wipes. **Owner's decision, 2026-08-15: accept for now, do
  not block BH-044 on it.** Documented as a known gap in `AuditService`'s own class doc rather than
  silently absorbed. BH-044.1 tracks the longer-term fix (owner's stated lean: move identity/
  reference fields like `bankId` out of `metadata` into real columns, keeping JSON for genuinely
  sensitive detail only) — unscoped, unprioritized, explicitly deferred.
...
| Date | Change | Why |
|---|---|---|
| 2026-08-15 | **BH-044 engineering built and reviewed; BH-044.1 opened and deliberately deferred with owner sign-off.** `AuditService.redactExpiredMetadata()` implements the owner's 08-15 retention decision, on branch `fix/bh-044-audit-log-redaction`, not yet merged. A 7-agent review before it shipped caught a real `CREATE INDEX CONCURRENTLY` migration deadlock (reverted to a plain index — an unbounded startup hang is worse than the brief write-lock CONCURRENTLY was meant to avoid), a test-isolation bug (an IT test mutating a live Spring-managed scheduled singleton with no teardown), and two smaller fault-tolerance gaps, all fixed before commit. The review also found redacting a Bank audit row permanently breaks the admin Bank audit tab after the retention window (`Bank` entities have no `entityId`; their only identity lives inside the metadata this sweep wipes) — presented as a real design gap, not silently absorbed. **Owner's decision: accept for now, don't block the PR on it.** BH-044.1 tracks the longer-term fix (owner's stated lean: move identity/reference fields out of `metadata` into real columns), unscoped and deliberately deferred | Same discipline as every other decision in this plan: state the tradeoff and record the decision the moment it's made, not after the fact. A redaction feature permanently deleting the only path to real audit history is exactly the kind of consequence that needs a named owner call, not an engineering judgment call made silently |

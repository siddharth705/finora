...
| **Open bug-hunt findings** | **0 Critical, 0 High from the numbered backlog.** BH-042/043/044 fully closed; BH-045 deliberately descoped; **BH-060 independently re-verified closed 2026-08-18** (re-ran its regression tests directly against current `main`, not just re-quoted — 64/64 green). **Real remaining gaps, all unticketed:** R-19 (`DashboardService`/`InsightsService` date-anchoring bugs, confirmed still live 2026-08-18 — see §7), the Bank-entity audit-lookup gap from BH-044's re-verification, and **PR #149** — a real, fully green, unmerged fix for eager `fileContent` loading across 5 services, sitting idle 26+ hours (§4) |
| **Baselined against** | `origin/main` @ `fef2762d` (re-baselined 2026-08-18; previous full baseline `a2f2cad`/2026-08-16, 86 commits behind — see §12). **CI not confirmed green at the actual current tip** — rapid overlapping pushes are cancelling each other's runs via the branch's own concurrency group; the last commit anywhere in the chain with a fully completed green run is `b545cd20`, five commits behind. Every individual PR passed its own full CI on its own branch before merging (a code-quality fact, not in question); what's genuinely unconfirmed is a single green run at the literal current HEAD. **New, untriaged: PR #176's post-merge run failed `Backend (Java 25)` on `MerchantLearningQueueIT`'s own premise-check assertion** — not yet independently re-run to tell a real regression from this repo's documented shared-context test-flake class (BH-058) apart |
...
### A real, ready fix sitting unmerged — PR #149 (found 2026-08-18)
...
`CapabilityCoverageService`, `StatementImportService`, `WorkspaceDashboardService`) exists only on
the still-open, fully green, mergeable **PR #149**, sitting idle 26+ hours with zero reviews. All
five services still call the entity-returning finder that eagerly loads up to 10 MB of statement
bytes into heap per call, confirmed live on current `main` by direct grep. This is the **third** time
in four days a real, ready, zero-risk PR has sat unmerged for no technical reason (`#130`, `#97`, now
`#149` — see §7 R-20). Memory corrected to match; recommend merging #149 as-is.
...
| R-20 | A fully green, zero-risk, mergeable PR sits unreviewed for a day or more before merging, for no technical reason | Low severity per-incident, **High as a pattern** | Observed 3 times in 4 days (#130, #97, now #149) | Real fixes (data export, regression-coverage upgrades, a 5-site eager-load memory bug) sit off `main` for no reason other than nobody clicked merge; one of these (#149) is a live-heap-usage bug still affecting every production request through the 5 call sites it touches, for as long as it stays unmerged | No process change proposed yet — naming the pattern is this pass's contribution; needs an owner decision on whether ready PRs should have a merge SLA | 🟠 New, process gap |
| R-21 | A new, unexplained CI failure surfaced on `main` itself, untriaged | Unknown — could be a real regression or a known-flake-class repeat | Observed once, 2026-08-18, not yet re-run in isolation | `MerchantLearningQueueIT.anEventAbandonedInProcessingIsReturnedToTheQueue` failed its own premise-check assertion (line 436) on PR #176's post-merge run — the stranded fixture the test expects to still be `PROCESSING` was found `PENDING` before the test's own recovery call ran, meaning something else reset it first. Could be a real regression, or a repeat of this project's already-documented shared-context test-isolation flake class (BH-058) | Re-run `MerchantLearningQueueIT` in isolation to establish real-regression vs. flake before deciding anything further | 🔴 Open, new, untriaged |

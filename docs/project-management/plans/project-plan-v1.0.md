...
| — | Gmail Sync (C1–C5.4: OAuth, sender trust gate, discovery, 6 merchant parsers, connection UI + review queue) | ✅ **Complete** — C5.4 merged 2026-08-15 (PR #122). **C6 Sprint 1 (C6.1 completion + C6.2) now in progress per D-17** | PRs #104–#122, all merged. Same "grew outside the phase table" pattern §12 already flags for other unweighted work — not yet in §1/§2, security review still owed (see 08-15 changelog). **D-17 (2026-08-15) overrides D-15/D-16: C6 begins now, running in parallel with launch blockers rather than waiting for GA.** Scoped to Sprint 1 only — C6.1 completion (sync-health/receipt-count visibility, review-queue confidence + reasoning) and C6.2 (Merchant Intelligence Dashboard, admin-only). C6.3/C6.4/C6.5/C6.7 remain held; C6.6 (OAuth verification) stays a launch requirement, not C6 scope |
...
**C6 Sprint 1 (D-17, 2026-08-15) is a deliberate exception to "off the critical path."** Unlike the
items above, C6 isn't waiting — the owner explicitly overrode D-15/D-16 and accepted the GA-timeline
risk of running it in parallel with everything on this critical path, not behind it. Scoped
narrowly: C6.1 (finish the Gmail connection/review UX) + C6.2 (Merchant Intelligence Dashboard,
admin-only). Does not touch anything on this diagram — different code areas (Settings.tsx,
GmailReview.tsx, a new admin analytics endpoint), so it adds engineering-time risk to the GA date
(the same person's hours split two ways) without adding a technical dependency between them. C6.3
(unknown-merchant learning), C6.4 (reconciliation), C6.5/C6.7 (already have their own proposals)
stay held, per D-17's own scoping.

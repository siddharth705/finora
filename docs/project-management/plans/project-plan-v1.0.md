...
**Baselined:** 2026-08-09 · **Re-baselined:** 2026-08-21 (52 merges since 08-17 folded in — see §12)
...
| | |
|---|---|
| **Overall completion toward v1.0 GA** | **83% → 86% — see §2's recomputed table.** Real progress across all seven workstreams since the 08-21 baseline: 13 of §4c's 46 live findings closed (including the 3 named Live Highs — Bug 04/07/10 — that were §10's specific QA-gate blocker), Security Sprint 3 merged, D-28 (referral/billing) fully shipped, the CI self-hosted-runner SPOF (R-6) resolved by moving to GitHub-hosted runners, and admin-portal MFA + rebrand complete. See §12's 2026-08-22 changelog entry for the full arithmetic |
| **Current phase** | Phase 4 (original bug hunt) complete. **Phase 4c (full-repo bug review) open — 33 of 46 original live findings remain (13 closed via PRs #227/#229/#234/#238), the 3 named Live Highs blocking §10's QA Complete gate are now closed.** Phase 5 (production readiness) **not started**. **Phase 2 (Transaction Boundary & Extraction Reliability)** is in progress as the parser-reliability track after §4a's metadata work — 2A–2D and 2E.1–2E.2 complete and merged (6 PRs), 2E.3+ and 2F–2H not started — see §4d and D-29; not a v1.0 gate, same as §4a |
| **Health** | **On Track on engineering, stalled on everything that actually sets the date** — see §8. R-18 (deployment failure) is now resolved (see Deployed row below); the remaining blockers are store enrolment, D-7 pricing, and D-12 privacy policy, none of which moved this week |
| **v1.0 scope** | Web + admin portal + **mobile** (D-2, 2026-08-09) |
| **Open bug-hunt findings** | **Original numbered backlog (BH-0XX): 0 Critical, 0 High still open** — unchanged, all P0/P1/P2 items from §4 remain closed or deliberately deferred. **§4c full-repo review: 33 of the original 46 live findings remain open** (13 closed since 08-21 via PR #227 Bug 04/07/10 — the 3 named Live Highs — PR #229 Bug 33/34/35, PR #234 Bug 14/18/36/40/47, PR #238 Bug 09/16 fixed + Bug 54 closed as no-defect), on top of 3 Criticals and up to 8 Highs found-and-fixed during the original review itself — full detail in [`BUG_REVIEW_REPORT.md`](../../quality/bug-reports/BUG_REVIEW_REPORT.md). **PR #230 (open, not yet merged) covers 4 more** (Bug 24/26/50/53, auth/security correctness) — not counted as closed until merged. **BH-061 remains open** (81% of sections / 100% of credit-card sections never get a usable account/card number — root cause in `PdfMetadataExtractor`) — Phase 1C/1C.1 (§4a) fixed the HDFC/Kotak/SBI slice of this exact root cause since BH-061 was filed, but the baseline has not been re-measured against the corpus, so BH-061 stays open rather than being asserted closed |
| **Baselined against** | `origin/main` @ `a73bee21` (re-baselined 2026-08-21; previous baseline `d5aec3d`, 52 PRs / ~100 commits behind). CI on the current tip fully green: Admin portal, Backend (Java 25), Cloudflare Pages ×2, End-to-end smoke, Mobile (Expo), User frontend all `success` |
| **Commits** | 900+ across 21 days (first commit 2026-07-31) |
| **Backend** | **3200/3200 tests green**, confirmed from the latest successful CI run's own log. Up from 2833 at the 08-16 baseline |
| **Clients** | frontend 57 test files / 569 tests · admin-portal 42 files / 314 tests · mobile 37 suites / 364 tests — all green, from the latest successful CI run's own log, not estimated (08-16's count was by file only; this pass has real test counts for the first time) |
| **CI** | green on `main`, **GitHub-hosted `ubuntu-latest` runners** (migrated off the self-hosted macOS runner in PR #224, `a1ff4bac` — see §7 R-6, now closed), smoke E2E blocking on every PR |
| **Deployed** | Yes — `app.finoratech.info` (Cloudflare Pages, 200 OK) + Railway backend (`api.finoratech.info/actuator/health` → `UP`) + Railway Postgres. **R-18 resolved**: GitHub's own deployment API now shows the latest deployment (`a73bee21`, current `main` tip) at `state: success` — production is serving the current commit, not a stale build as it was on 08-16 |
| **Store enrolment** | **No new evidence of movement found this pass either — still the single most stagnant item on the plan.** Apple: submitted as Individual, still not confirmed complete. Google Play: still not started. This can only be confirmed by the owner directly (App Store Connect / Play Console are not visible from the repo) |
| **Production-readiness audit** | **Complete** (2026-08-11), **scope now five features further behind**: Gmail C6.1/C6.2, the account-lifecycle trilogy, Transaction Explanation (all noted 08-16) plus Sign-in with Google/Apple (a new account-takeover-adjacent auth surface) and the Premium Layer entitlement architecture (PR4-A, new financial-data-adjacent surface), neither security-reviewed by an equivalent pass — though both did get their own inline self-review treatment per §12 |
...
|---|---|---|---|---|---|
| 1 | Core product (auth, ledger, accounts, budgets, goals, dashboard, reports, admin portal) | 20% | **95%** ▲ | 19.0 | Bug 10 (`GET /recurring` mutates) fixed PR #227; Bug 09/16 fixed + Bug 54 closed no-defect (PR #238). Admin-portal rebrand (#228) + dark mode/password toggle (#243) + MFA (#246, login challenge + self-service enrollment + recovery codes, inert until `app.admin-mfa.enabled=true`) all shipped. Remaining known gaps: Bug 29 (Swagger UI unauthenticated off-`prod`), Bug 50/53/57 — Bug 50/53 covered by open, unmerged PR #230 |
| 2 | Import pipeline (M1 reliability + M2 at-scale) | 20% | **85%** ▲ | 17.0 | **Bug 04 fixed** (PR #227, LIVE High — unbounded category name rolling back an entire import, the flagship import-correctness gap this row previously carried), plus Bug 33/34/36/56-adjacent items (PR #229/#234). Offsetting, unchanged: BH-061 still open (81%/100% of CC sections lack a usable card number), `PdfTableLocator`/`imports/product/` still never fully reviewed |
| 3 | **Financial correctness defects** | 10% | **92%** ▲ | 9.2 | **Bug 35 fixed** (PR #229 — `BudgetService.upsert` no longer always reports spend as zero) and Bug 33/34 (import/budget correctness) closed alongside it. The three Criticals fixed during the original §4c review still stand as the largest single gain this row has had |
| 4 | Security & privacy | 12% | **97%** ▲ | 11.64 | **Security Sprint 3 merged** (PR #188, `5dcf8600` — SEC-05/10/11/12/16), closing the "built, not yet merged" gap this row carried at the last baseline. **Bug 07 fixed** (PR #227 — 429 responses now carry CORS headers). Admin MFA (SEC-03) moved from "backend complete, enforcement deferred" to fully shipped including the UI follow-up (#246) — see row 1. Bug 26 (`PhoneVerificationFilter` fails open) remains open, covered by unmerged PR #230 |
| 5 | Testing & QA readiness | 12% | **93%** ▲ | 11.16 | Suite counts unchanged this pass (not re-measured), but the QA-gate-blocking §4c Live Highs (Bug 04/07/10, §10) are now closed and 13 of 46 live findings resolved overall — a real QA-readiness gain even without a fresh test-count pull |
| 6 | Infrastructure & production readiness | 14% | **64%** ▲ | 8.96 | **CI self-hosted-runner SPOF (R-6) resolved**: PR #224 (`a1ff4bac`) moved all workflows to GitHub-hosted `ubuntu-latest` — confirmed via `runs-on:` in every `.github/workflows/*.yml`, zero self-hosted/macOS references remain. Reason it mattered: the repo is now public, and a self-hosted runner executing fork-PR code under the registering machine's own user account is a real exposure GitHub advises against. Everything else genuinely unchanged: no restore drill, no load test since 08-16, no secret manager |
| 7 | Mobile app | 12% | **76%** — | 9.12 | No change to the store-enrolment gate (§1/§9a) this pass. Minor shipped fixes (Face ID AppState re-trigger #220, `expo-screen-capture` iOS keyWindow patch #219) don't move this row — still no confirmed physical-device run |
| | **Total** | **100%** | | **86.08 → 86%** | **Re-baselined 2026-08-22 — see §12 changelog for the full row-by-row arithmetic.** 19.0+17.0+9.2+11.64+11.16+8.96+9.12 = 86.08, rounding to 86%. Net **+3** over the 08-21 baseline (82.9 → 86.08), driven by 13/46 §4c findings closed (incl. the 3 Live Highs blocking §10's QA gate), Security Sprint 3 merged, and the CI SPOF closed — mobile (row 7) is the one row held flat, since none of this pass's fixes touch the store-enrolment constraint that actually gates the date |
...
**The pattern mostly holds, with one real break.** Defect and feature work keeps closing at high
volume and it keeps not moving the date, because the date was never gated on it — store enrolment
and D-7/D-12 are still the actual constraint, unchanged this pass. Workstream 6 (infrastructure) did
move for the first time since 08-10, from 57% to 60%, but only because R-18's deployment failure
resolved — a monitoring/ops fact, not new backup/DR/load-test/secret-manager work. The underlying
claim (code velocity isn't what sets the date) still holds; the number just stopped being frozen.

**Velocity — stale, not recomputed this pass.** The figures below are from the 08-09/08-10 baseline
(489 commits/9 days) and were already behind §1's own commit count at the 08-16 re-baseline (750+);
§1 now shows 900+/21 days. Recomputing velocity honestly needs the same per-day PR/commit breakdown
this paragraph originally used, not a headline ratio — left as a flagged gap rather than asserting a
new number without doing that work. 489 commits / 9 days ≈ 54 commits/day; the last three days ran 105, 106 and ~40, at
...
| **4** | **Hardening & Defect Remediation** (original numbered BH-0XX backlog) | 🟡 **~98% — effectively complete, 1 item remains** | Corrected 2026-08-14, updated 08-15, corrected again 08-16 — all P0–P3 findings closed, accepted, or decided (§4). **`BH-044` confirmed fully closed 2026-08-16** (decision + engineering both shipped, PR #121 `52072e3`, independently re-verified — this row previously said the opposite, contradicting §1's already-correct note; see §12 changelog). `BH-045` remains deliberately descoped by owner decision, not an open engineering item. Not marked ✅ Complete only because the Bank-entity audit-lookup gap BH-044's re-verification surfaced (§4) is new and undecided. **Not the same backlog as Phase 4c below** |
| **4b** | Security review — hardening backlog | 🟡 **~75%** | Sprints 1/2/4 of 4 merged (#186/#187/#189); Sprint 3 built, PR #188 open, not merged. See §4b |
| **4c** | **Full-repository bug review (new, 2026-08-21)** | 🟡 **~19% — review complete, remediation barely started** | 58 findings, 46 live (3–4 Live High). 3 Criticals + up to 8 Highs found **and fixed** during the review itself; the 46 live findings (mostly Medium/Low) are undecided backlog, not yet sequenced by the owner. See new §4c |
...
| — | **Phase 2 — Transaction Boundary & Extraction Reliability (new, 2026-08-21, D-29)** | 🟡 **2A–2D complete, 2E.1–2E.2 complete, 2E.3+ and 2F–2H not started** | Follows §4a's metadata-reliability work (Phase 1). Scoped 2A–2H (start/end detection, non-transaction noise, row completeness/correctness, multi-page continuation, reconciliation, import-result reporting); 2E further split by the owner into its own 2E.1–2E.5 investigate-then-design-then-implement sub-phase (header reconstruction, narration, balance semantics, HSBC/IOB classification). PRs #222/#226/#231/#236/#239/#245, all merged. Investigation-first throughout — see §4d |
...
6. **SEC-03** — admin-portal MFA (TOTP or WebAuthn/passkeys + recovery codes) — closing a gap on an
   ...
   enroll via a direct API call with no way to complete the challenge through the web app. **Status
   updated 2026-08-22: the admin-portal MFA UI follow-up shipped in PR #246 (`483357da`)** — login
   challenge, self-service enrollment, and recovery codes. SEC-03 is now fully built end to end;
   enforcement still stays behind `app.admin-mfa.enabled` (defaulting `false`) until the owner flips
   it, per the same lockout-risk reasoning recorded above — this is a deliberate activation gate, not
   remaining engineering work.

**Sprint 3 — Security maturity. Merged 2026-08-22** (PR [#188](https://github.com/siddharth705/finora/pull/188), `5dcf8600`, covers SEC-05/10/11/12/16) — closed as of this re-baseline, no longer the open item this section carried at the 08-21 baseline
...
## 4c. Full-repository bug review (new, 2026-08-21)

**Why this section exists.** [`BUG_REVIEW_REPORT.md`](../../quality/bug-reports/BUG_REVIEW_REPORT.md)
landed on `main` (correcting its own tally in [PR #195](https://github.com/siddharth705/finora/pull/195),
merged 2026-08-21) with no reference anywhere in this plan — the same gap §4a and §4b were each
written to close for their own workstreams. This is a **separate review from the original 2026-08-08
bug hunt** (§4): different scope (every top-level directory, not a targeted sweep), different
numbering (`Bug NN`, not `BH-0XX`), and it was not cross-checked against the original backlog for
overlap before this re-baseline — treat the two as independent until someone does that reconciliation.

**Scope and method.** Every top-level directory (`backend/`, `frontend/`, `admin-portal/`, `mobile/`,
`scripts/`, `.github/workflows/`, `.husky/`, root config). Read-only — no code was modified, no PRs
were opened by the review itself. The working tree moved underneath the review as concurrent PRs
merged, so all 58 findings were re-checked against the moving tree by code signature, not by the
line numbers the findings were originally written against.

**Result: 58 findings — 3 Critical (all fixed during the review), 10 High (5 fixed, 1 corrected to
Medium, 4 live), 20 Medium (2 fixed, 18 live), 25 Low (1 fixed, 24 live). 46 total live at the
2026-08-21 baseline, 11 fixed during the review itself.**

**Updated 2026-08-22: 13 of the 46 live findings closed via four merged PRs, 33 remain live.**
PR #227 (`d6d3444b`) fixed Bug 04/07/10 — the exact 3 named Live Highs §10's QA-gate criterion called
out by name as blocking. PR #229 (`492da242`) fixed Bug 33/34/35 (import/budget correctness).
PR #234 (`7e120d13`) fixed Bug 14/18/36/40/47 (operational/observability/audit). PR #238 (`fae69194`)
fixed Bug 09/16 and closed Bug 54 as no-defect (investigated, not reproducible) — 12 code-fixed + 1
no-defect-closed = 13. **PR #230 (open, not yet merged) covers 4 more** — Bug 24/26/50/53
(auth/security correctness) — not counted as closed until it merges. Bug 30 remains deliberately
deferred (same root cause as BH-060, do not touch).
Three Criticals fixed during the review itself are worth naming, since they are real defects this
plan had no prior record of: **Bug 01** (the entire merchant-learning durable queue built for the
Import Reliability Milestone was never actually called — `MerchantLearningService.confirm` still ran
synchronously inside the caller's transaction, so a learning-table race could still roll back an
already-reviewed statement's entire import), **Bug 02** (a client-supplied closing balance was
written to `Account.balance` with no validation — closed by a new `ClosingBalanceGuard`), **Bug 03**
(every UPI/NEFT/IMPS/ATM transaction collapsed into one merchant — closed by `PaymentRailTokens`
tokenising both sides of the rail).

**Live findings worth naming here** (full detail, evidence and reproduction steps for all 46 in the
report itself):
- **Bug 04 (High)** — an unbounded category name from a statement rolls back the entire import.
  Financial-correctness-adjacent; not previously ticketed.
- **Bug 07 (High)** — rate-limit `429` responses carry no CORS headers, so browsers never surface them.
- **Bug 10 (High)** — `GET /api/v1/recurring` is a write endpoint.
- **Bug 35 (Medium)** — `BudgetService.upsert` always reports spend as zero.
- **Bug 53 (Medium)** — the refresh-token cookie is inert whenever the API is not on the same
  registrable domain as the SPA (relevant to `app.finoratech.info` / `api.finoratech.info`'s actual
  cross-subdomain deployment — not yet checked against this project's real cookie-domain config).
- The remaining ~41 live findings span rate-limiting, CORS, import parsing (Bug 30/33/34/56 —
  overlaps §4a's parser-reliability territory), auth edge cases, and frontend polish. See the report
  for the full list; not reproduced here to avoid this plan drifting out of sync with it.

**Not sequenced.** Unlike §4b (which the owner explicitly prioritized into 4 sprints the same week it
landed), these 46 findings have **no owner sequencing decision yet** — this re-baseline records their
existence and severity distribution, not a remediation order. That decision belongs in §11 once posed.

**Two active worktrees at time of this re-baseline** (`resolve-sprint3-conflict`,
`bug-review-tally-fix`) suggest at least one other concurrent session is already working this area —
check `gh pr list` before starting remediation here to avoid duplicating in-flight work.

---

## 4d. Phase 2 — Transaction Boundary & Extraction Reliability (new, 2026-08-21, D-29)

**Not a v1.0 release gate**, same standing as §4a — parser-reliability quality work, tracked
separately from launch. See D-29 in §11 for the full decision record.

**The question Phase 1 (§4a) didn't answer.** Phase 1 established whether Finora can correctly read
a statement's *metadata* — holder, account/card number, period, due date, total due, acquisition
provenance. Phase 2 asks whether Finora can correctly identify the *transaction table* itself: where
it starts, where it genuinely ends, and what separates a real transaction row from bank-generated
noise (ads, rewards content, T&Cs, interest-rate illustrations) that can follow it in the same
document.

| Area | Objective | Current state |
|---|---|---|
| 2A | Transaction start — identify where real transactions begin | ✅ **Investigated** — `transaction-boundary-phase2a-investigation.md`, PR #222 |
| 2B | Transaction end — stop at the last real transaction | ✅ **Fixed** — `STATEMENT_CLOSING_MARKER` extended into `PdfTableLocator`'s main header-based loop, shared one-way suppression mechanism (`TRANSACTION_TABLE_CLOSED`). PR #226 |
| 2C | Non-transaction content — prevent ads/rewards/T&Cs/illustrations from becoming transactions | ✅ **Fixed for 2 more banks** — Kotak's `TRANSACTION_TABLE_TOTAL_CLOSED`, ICICI's `MITC_SECTION_CLOSED`, ordered `TrailingContentTrigger` list. PR #231 |
| 2D | Row completeness — every real transaction becomes a staged row | ✅ **Investigated** — cross-checked all 21 ground-truthed real documents, 20/21 row-complete (the 1 exception is Shivani_HDFC's RD installment gap, correctly deferred behind the ground-truth-model ADR-005 gate, not a Phase 2 defect). Confirmed but explicitly did NOT fix a header-reconstruction-collapse bug on 3 documents (SBI, Statement.pdf/IOB, HSBC DB.pdf) — see 2E below. `row-completeness-phase2d-investigation.md`, PR #236 |
| 2E | Row correctness — date/description/debit/credit/balance belong to the same transaction | 🟡 **In progress, reframed by the owner into its own sub-phase** — see the dedicated breakdown immediately below. 2E.1 and 2E.2 complete; 2E.3+ not started |
| 2F | Multi-page statements — correctly continue/terminate the transaction table across pages | ⬜ Not started |
| 2G | Transaction reconciliation — extracted transactions vs. statement totals | ⬜ Not started. Existing validators (`BalanceChainValidator`, `StatementTotalsValidator`) can likely become the evidence layer, not built from scratch |
| 2H | Import result — a trustworthy "what was imported and what wasn't" explanation to the user | ⬜ Not started. `ImportReliabilityStatus` (§4a's evidence-based status, PR #184) is a foundation to extend, not a new mechanism |

**Sequencing discipline, validated twice more during 2E.1.** Real-corpus investigation before any
fix continued to catch wrong initial theories: the header-reconstruction root cause on SBI was first
hypothesized to be an embedded date-range defeating a wrap check (wrong, disproved by direct trace),
then correctly diagnosed as `mergeHeaderLines` having no model for a header whose columns are
*partitioned* (not refined) across physical lines. **Do not change row grouping
(`groupIntoRows`/`PdfTableLocator`) until real-corpus evidence proves where the actual signal is** —
still the standing rule for whatever of 2F/2G/2H starts next.

### Phase 2E — Transaction Intelligence Reliability Audit (owner-reframed, 2026-08-21/22)

After 2A–2D, the owner posed the phase's governing question directly: **"For every transaction
Finora imports, can we prove it came from the correct table, with the correct columns, and with
complete narration?"** — and broke row-correctness (2E above) into its own sub-phase, investigate-
then-design-then-implement, same discipline as every phase before it.

| Sub-phase | Scope | Status |
|---|---|---|
| 2E.1 | Header Reconstruction Recovery — why headers collapse when a table's real header prints across multiple physical lines in a shape `mergeHeaderLines` has no model for | ✅ **Investigated + designed.** Root-caused SBI CC Section 1 precisely (a line supplying only one column, its neighbor supplying the rest, with zero X-overlap) and Statement.pdf/IOB (the same gap plus an independent `PdfTextExtractor` fusion artifact). Reproduced HSBC DB.pdf's real OCR pipeline directly, resolving a stale "0 runs" reading as a diagnostic-tool gap, not a regression. Two design docs followed the owner's own review corrections (a `HeaderQualityValidator` gate keyed on the ACCEPTED header's quality, not on whether a merge attempt failed — traced against the real code to show SBI's own accepted header was never the subject of a failed merge at all). `header-reconstruction-phase2e1-investigation.md`, `header-reconstruction-design.md`, `header-reconstruction-regression-corpus.md` — PR #239 |
| 2E.2 | Header Quality Gate + Reconstruction Engine — first implementation, scoped narrow per explicit owner instruction ("do not attempt SBI + IOB + HSBC simultaneously; use SBI as the first proof the architecture works") | ✅ **Shipped, held to SBI's exact shape.** New capability `HEADER_RECONSTRUCTED`: composes a single orphaned header fragment from the line immediately above an accepted-but-weak header, validated by actually bucketing the section's real rows (no two raw values may collide into one column, and the date must parse) rather than by cell count or vocabulary. Three real bugs caught and fixed before any of it reached a test: the gate originally preempted an already-correct, older recovery mechanism on a real ICICI header; the same reordering fix also closed a section-merge risk (SBI's primary/supplementary cardholder sections share an identical true header shape); two successive validation designs (cell-count, then date-only) each passed their own tests while missing a real anchor-misalignment corruption. **Deliberately scoped to a single-cell fragment** (`nonBlankCount(above) != 1`) after verifying, on a real ICICI savings statement, that widening it to a genuine multi-cell second tier recovers some real transactions but leaves the statement's own aggregate balance-total check failing — the owner's explicit call was to hold this phase to SBI-only rather than ship that partial result. PR #245 |
| 2E.3 | Narration correctness (owner's original framing: "ICICI savings description is wrong for 11 rows") | ⬜ Not started. Flagged as unverified against this project's own Phase 2D finding (that document was 11/11 row-complete with no description-accuracy check ever run) — treat as a hypothesis to check, not an established fact, before any fix |
| 2E.4 | Balance semantics (owner's original framing: "CBI opening balance detected as 47.77") | ⬜ Not started. Also flagged as unverified — never investigated |
| 2E.5 | HSBC/IOB classification | ⬜ Not started. Statement.pdf/IOB's header failure is root-caused (2E.1) but not fixed; HSBC DB.pdf's is root-caused via the real OCR pipeline but not fixed. Both need the general, multi-tier header-composition case 2E.2 deliberately deferred, not the single-fragment engine already shipped |

**The general multi-tier composition case is the most likely next step inside 2E**, since it's what
both 2E.2's own deferred scope (ICICI) and 2E.5 (IOB, HSBC) need — but the owner has not yet given
that explicit go-ahead; the last instruction was "hold this phase to SBI-only," not a decision about
what comes after.

**Status: 2A–2D complete and merged. 2E.1–2E.2 complete and merged. 2E.3–2E.5 and 2F–2H not started.**
...
| R-6 | ~~Single self-hosted macOS runner is a CI SPOF~~ | — | — | — | **Closed 2026-08-22.** PR #224 (`a1ff4bac`) migrated every workflow to GitHub-hosted `ubuntu-latest` runners — confirmed via `runs-on:` in all `.github/workflows/*.yml`, zero self-hosted/macOS references remain. Also closes the security exposure of a self-hosted runner executing fork-PR code under the registering machine's own user account, now that the repo is public. GitHub issues #101 and #94 (self-hosted-runner DB/container lifecycle and dev-Postgres dependency) are now likely stale/closeable — flagged as a follow-up, not closed here | ✅ Closed |
...
| R-18 | ~~GitHub's deployment status on the current `main` tip reads "Deployment failed" (Railway)~~ | — | — | — | **Closed 2026-08-21.** GitHub's deployment API now shows the current `main` tip (`a73bee21`) deployed at `state: success` — production is serving the latest commit again, no owner action was needed to resolve it | ✅ Closed |
...
| R-20 | §4c's full-repository bug review found 46 live findings (3–4 Live High) with no owner sequencing decision | Medium, reduced | Confirmed, 33 of 46 still live as of 2026-08-22 | **Downgraded 2026-08-22**: the 3 named Live Highs (Bug 04/07/10) that made this risk urgent are closed (PR #227), along with 10 more Medium/Low findings across 3 further PRs. 33 remain, still unsequenced by the owner as a batch, but no longer includes a High-severity item | PR #230 (open) will close 4 more (Bug 24/26/50/53) once merged; pose the sequencing question for the rest the way §4b's 4-sprint order was posed | 🟡 Open, downgraded |
...
| **QA Complete** | 🔴 | **Updated 2026-08-22 — the specific Live High blocker named at 08-21 is now closed.** Original numbered bug-hunt backlog (§4): 0 Critical, 0 High, unchanged (P0–P3 all closed/accepted except BH-044's engineering, not yet built, and BH-045, deliberately descoped). §4c's independent full-repo review: **Bug 04/07/10 — the 3 Live Highs criterion 1 ("zero open Critical or High defects") was failing on — are fixed (PR #227)**. Per the original numbered-backlog+§4c reading, no Live High currently blocks criterion 1; PR #230 (open) may surface whether any of Bug 24/26/50/53 rises to High once merged. Still red on everything else: full E2E still not in CI; cross-browser still never green; the 7 named test gaps below are unverified this pass (carried forward, not re-checked); 33 §4c Medium/Low findings remain live and unsequenced (§7 R-20) |
...
| ~~**D-21**~~ | ~~After the phone-verification/account-recovery redesign (PR #143) shipped, owner pitched "First Run Experience" -- welcome screen → choose setup path (import/Gmail/manual) → first insight generated → dashboard becomes alive, framing the empty ₹0 dashboard as the next activation problem. Build it now, or hold for post-GA?~~ | — | ✅ **Decided 2026-08-16: explicit override, start now -- same shape as D-17.** Audited before framing the ask, same discipline as D-17/D-18/D-19/D-20: `Dashboard.tsx` has no dedicated empty-state flow today (generic greeting + one import card), so the idea is real, unbuilt scope, not already-there-and-unrendered the way most of D-19's audit turned out. But it sits off today's actual critical path (§5) -- production readiness (Phase 5, not started), R-18 (latest `main` tip failed to deploy on Railway), D-10 (lining up the 12 Play testers), D-12 (privacy-policy data controller, holding on a lawyer's view) -- and per D-1, production has zero real users right now, so the empty-dashboard problem isn't costing anything today the way those are. Owner's call: **explicit override, accept the added GA-timeline risk** (D-17's own precedent for exactly this tradeoff), not held as a parked proposal. **Scope, not the whole pitch at once** (same "narrowest slice first" discipline as D-17's Sprint-1-only and D-19's Step-1-only): Step 1 is the empty-state Dashboard treatment (welcome moment + 3 setup-path CTAs: Import → existing Import page, Connect Gmail → existing Settings flow, Add manually → new modal) plus wiring up manual transaction entry, which turned out to already exist end-to-end and unused -- `TransactionController.create()` (`POST /api/v1/transactions`) and `transactionsApi.create()` in `endpoints.ts` both already work, `grep`-confirmed zero call sites anywhere in the frontend UI, the same "backend maturity ahead of UI maturity" pattern D-19's audit found repeatedly elsewhere. **"First insight generated" is explicitly held, not part of Step 1** -- no existing signal for "this was the user's first successful import" and no design yet for what the insight should say, unlike the other three pieces which need no new design. |
| **D-22** | **Owner asked for a from-screenshot redesign of the D-21 empty-state Dashboard (per-section empty states instead of one welcome gate, collapsible left sidebar) — built and verified same session. Owner then reviewed it against Finora's product direction and proposed a 4-PR sequence (Dashboard redesign → Design System → Real Dashboard Intelligence → Premium Layer). Merge now, and is that sequence right?** | — | ✅ **Decided 2026-08-16: yes, merge this PR, then Design System before any more page work — owner's own sequencing, recorded as stated.** Two things checked against actual code before agreeing, same discipline as every other D-row: (1) owner flagged risk of "fake premium elements" (Finora Plus banner, Upgrade CTA, premium analytics) creeping into the redesign from the reference screenshot — verified none were built; D-21 Step 1 already excluded the screenshot's "Finora Plus ✨"/Premium Plan upsell for the same reason (D-7 pricing unresolved), so this caution was already satisfied, not a new instruction. (2) owner's Financial Health Score critique ("35 / Needs Attention feels negative for a new user") checked against `Dashboard.tsx`/`DashboardService.computeHealthScore`: the section is fully hidden for a true zero-transaction user (`{!isEmpty && (...)}`, D-19 Step 1's own gate) — a brand-new signup never sees a harsh score — but `computeHealthScore` has no minimum-data floor, so the critique is real for the state it actually applies to: a handful of transactions, still-thin data, genuinely low weighted score. Narrower gap than described, not a false alarm. **Owner's sequence adopted as-is**: PR1 (this session's redesign, merge now) → PR2 **Design System** (`frontend/src/design-system/`: `FinoraCard`, `MetricCard`, `EmptyState`, `SectionHeader`, `QuickActionCard`, `ChartContainer`, `PremiumBadge` — reusable primitives before Transactions/Reports/Investments/Goals drift into inconsistent styles) → PR3 **Real Dashboard Intelligence** (real cash-flow timeline from transactions, the Financial Health Score minimum-data floor from the audit above, a "Financial Journey" milestone section) → PR4 **Premium Layer** (`PremiumFeatureGate` → `SubscriptionPlan` → `FeatureFlag` architecture, gated behind D-7 pricing same as before — **prepare the architecture, do not ship priced UI**, consistent with D-7's own still-open status). Owner's premium onboarding-flow pitch (signup → welcome → connect → first insight → dashboard unlocked) and the mobile design-language alignment pitch are **not sequenced yet** — noted, not scoped into any of PR2-4 above; next to raise once PR2 lands. |
| **D-23** | **Owner asked why "Sign in with Google" doesn't appear on the register/login screen (a screenshot). It doesn't exist — the only Google integration is Gmail Transaction Sync, a Settings-page feature for an already-logged-in user, unrelated to authentication. Add real Google sign-in to registration and login, "same as mobile app" — a genuinely new, security-sensitive feature, not a bug fix.** | — | ✅ **Decided 2026-08-16, phased.** Investigated before designing, same discipline as every other D-row: `AuthController.java` already carries a `// TODO Phase 2: /oauth/google callback endpoint` placeholder, and `GoogleOAuthProperties`'s own doc comment states outright "this is not Sign in with Google" — anticipated, never built. Three real security/product forks surfaced and put to the owner rather than assumed: **(1) account linking** when a Google email matches an existing password account — **owner chose auto-link on Google's verified email** (the same trust basis "Sign in with Google" uses everywhere), not reject-and-redirect. **(2) phone verification** for Google signups, who arrive with no phone number at all — **owner chose to keep it mandatory for everyone**, routing a new Google-created account through the same OTP step every other signup already goes through, rather than carving out a permanent gap in the one security control that currently covers every account uniformly. **(3) sign-in flow** — **owner chose Google Identity Services** (a real Google-rendered button, ID token posted to a new backend endpoint, verified via `GoogleIdTokenVerifier`) over a popup+authorization-code flow matching Gmail sync's own pattern; owner already has a Google Cloud OAuth client for this, separate from the Gmail-sync one. **Phased: Phase 1 (web) first, Phase 2 (mobile) reusing the same backend endpoint** — mobile's Expo config-plugin + dev-client workflow (already proven with `@react-native-firebase`) fits `@react-native-google-signin/google-signin` cleanly, no prebuild/eject prerequisite; the backend ID-token verification is identical for both platforms, only the client-side SDK differs. **Implementation notes found during scoping, not owner decisions**: no schema migration needed (`phone_number` is already nullable; a new Google-only account gets a random, cryptographically unguessable `password_hash` rather than a nullable column); the existing `PhoneChangeService`/`phone-change/start\|verify-otp\|complete` flow (already reachable by a phone-unverified user per `PhoneVerificationFilter`'s own allowlist) is reused as-is to collect a Google signup's first phone number, rather than building a new screen. **Phase 1 (web) built 2026-08-17**, [PR #160](https://github.com/siddharth705/finora/pull/160), pending CI/review. Backend suite 2965/2965, frontend suite 500/500. **Three self-review passes before this was ready to ship, each catching a real bug a green test suite alone would not have**: (1) `GoogleIdTokenVerifierConfig` returning `null` from a `@Bean` method when unconfigured broke Spring's whole application context — constructor autowiring treats `null` as "no candidate" for a required dependency, not "inject null"; fixed to always construct a real verifier and check configuration at call time instead. (2) The auto-link decision above assumed an existing account's email carried the same proof Google's own claim does — it didn't: password registration never verified email ownership, so an attacker could pre-register a victim's email and silently inherit any account the victim later reached via Google (the classic OAuth "pre-hijacking" attack), a gap not surfaced when the account-linking fork was originally decided. Closed by adding real email verification for password registration (new `User.emailVerified`, a token-mint/verify flow, `POST /api/v1/auth/verify-email`) and gating the auto-link on it. Same pass also closed: `/auth/google` had no rate limit, and Google's self-reported display name was stored unchecked. (3) The unverified-account rejection branch wrote a token then threw — without `noRollbackFor = ApiException.class` Spring's default rule silently rolled that write back, the same bug class this codebase's `login()`/`refresh()` already had to fix twice before; proven with a real Testcontainers transaction test that fails without the fix. Same pass also found `loginWithGoogle()` had no check against the admin's registrations-enabled kill switch — closing public signups left Google sign-in as an unrestricted back door — now fixed. **Phase 2 (mobile) built 2026-08-17** — see the D-26 row immediately below for the combined Google+Apple build (D-26 widened Phase 2's scope before any of it was written, so the two shipped together, not sequentially). |
| **D-26** | **With D-23 Phase 1 (web) merged, owner asked what's next — surfaced before scoping Phase 2 (mobile): Apple's App Store Guideline 4.8 requires "Sign in with Apple" as an equivalent option once an app offers a third-party/social login (Google included) to authenticate its primary account. The exemption only covers an app that uses *exclusively* its own account system — which the iOS app will no longer qualify for the moment Google Sign-In lands there. Confirmed via live search against Apple's current published guideline, not assumed from training data. Should Phase 2 build Apple Sign-In alongside Google Sign-In for iOS, or scope around it?** | — | ✅ **Decided 2026-08-17: build both together for iOS; Android gets Google Sign-In alone.** Google Play carries no equivalent requirement, so Android's Phase 2 scope is unaffected — only iOS needs the pair. **Consequence for Phase 2's scope, not yet re-estimated**: what D-23's own row described as "Phase 2 reuses the same backend endpoint, only the client-side SDK differs" now also needs a second backend verifier (Apple's own ID-token/JWKS verification, parallel to `GoogleIdTokenVerifierService`) and a second native SDK (Sign in with Apple) wired into the iOS app alongside `@react-native-google-signin/google-signin` — real additional engineering D-23's original phasing didn't anticipate, not a relabeling of existing scope. **Built 2026-08-17.** Backend: `AppleIdTokenVerifierService` (mirrors `GoogleIdTokenVerifierService`, resolves Apple's JWKS via `com.auth0:jwks-rsa` since Apple ships no first-party Java verifier the way `google-api-client` does), `POST /api/v1/auth/apple`, its own rate limiter and `APPLE_LOGIN_CLIENT_IDS` config. `AuthService.loginWithGoogle`/`loginWithApple` were unified into one shared `loginWithOAuthIdentity` method — same account-hijack protection, same registrations-enabled gate, same audit trail, parameterized only by provider label — rather than hand-copying the security-sensitive body a second time. Mobile: `@react-native-google-signin/google-signin` + `expo-apple-authentication` installed and wired into `app.config.ts` (Google's plugin reads its client config from the SAME `GoogleService-Info.plist`/`google-services.json` already downloaded for `@react-native-firebase/auth`'s phone-OTP flow, no second credential-download convention introduced); both official brand-compliant button components (`GoogleSigninButton`, `AppleAuthenticationButton`) wired into `LoginScreen`/`RegisterScreen`, Apple's gated to iOS only. Backend suite 3000/3000 (17 new tests: `AuthServiceAppleLoginTest`, `AppleIdTokenVerifierServiceTest` against a locally-generated RSA keypair and a mocked `JwkProvider`, since no real Apple-signed token is obtainable in a test). Mobile suite 324/324 (27 new tests). **Self-review finding, fixed before commit**: `AppleAuthRequest.fullName`'s DTO-level `@Pattern` validation would have hard-failed the ENTIRE sign-in request (a 400, no session) on any value Apple's own name formatter could plausibly return that didn't match — including an empty string, from a `fullName` components object that's non-null but has every field null — rather than the safe email fallback `AuthService.sanitizeOAuthDisplayName` already provides one layer down. Removed; the mobile client also guards the empty-string case itself (`fullName \|\| undefined`) as defense in depth. **Known, pre-existing, explicitly out of scope**: mobile has no reactivation-flow UI for ANY sign-in method yet (web's `Login.tsx` shows a one-click reactivate step when `login()`/`loginWithGoogle()` returns `AUTH_ACCOUNT_DEACTIVATED`; mobile's `LoginScreen` has no equivalent for password login either, so Google/Apple sign-in on mobile match that existing gap rather than introduce a new inconsistency — a deactivated account hits a generic error message on every mobile sign-in path today). Real client credentials (Google Cloud OAuth client ids for Android/iOS, an Apple Services ID + Sign In with Apple private key) still need registering by the owner in their own developer consoles before this works end-to-end on a real device — same "unconfigured degrades to a 503, nothing else breaks" posture as every other integration in this codebase, verified structurally rather than against live credentials. |
| **D-27** | **D-22's PR3 sequencing named PR3-D as "Analytics — track signup → first import → first budget → first goal" in one throwaway line, never scoped in detail. With PR3-A/B/C all merged, what should PR3-D actually be?** | — | ✅ **Decided 2026-08-21.** Audited before asking, same discipline as every other D-row: the admin portal already has real precedent (`AdminOperationalDashboardController`, `AdminPlatformAnalyticsController`), both admin-only, both reusing an existing permission rather than minting a new one -- but zero funnel/cohort/conversion-tracking code exists anywhere (`AuditService` is rich enough to build one from, but no `GROUP BY date`/cohort query exists in this codebase at all). `FinancialJourneyService` (D-25 PR3-B) already computes the same four raw milestones per-user -- a funnel needs a genuinely new AGGREGATE query across the whole `users` table, not N calls to that service. Two real forks put to the owner rather than assumed: **(1) placement** -- admin-only (matching 100% of existing precedent) vs. user-facing on the end-user Dashboard. **Owner chose Admin Portal.** **(2) shape** -- a simple snapshot (one live query, no new infrastructure) vs. a cohort/time-series (week-over-week conversion trends, needing a scheduled pre-aggregation job that has no precedent anywhere in this codebase yet). **Owner chose the simple snapshot.** Built same day: `GET /api/v1/admin/dashboard/activation-funnel` (reuses `PLATFORM_STATS_VIEW`), each stage a distinct-user count against `budgets`/`goals`/`statement_imports` counting users who EVER reached it (not just currently-active), `signedUp` reusing the Operational Dashboard's own `totalUsers` definition so the two figures can't disagree. [PR #200](https://github.com/siddharth705/finora/pull/200), all 7 CI checks green, browser-verified live (0/0/0/0 on a fresh platform, then real counts after creating a budget+goal via the actual API). This closes D-22's full 4-part PR3 sequence. |
| **D-28** | **With PR3 fully closed, owner said "start pr4" (D-22's Premium Layer). Two real, unreconciled tensions found before writing any code: (1) D-22 approved PR4 by name on 2026-08-16, but a separate decision the SAME day put "Plus/Premium billing/entitlements" into post-GA scope, deferred "unless individually promoted into the launch plan by explicit future decision" -- neither entry references the other. (2) D-22's own PR4 scope note ("prepare the architecture, do not ship priced UI") undersells what already exists: `docs/proposals/billing-subscription-entitlements-proposal.md` is a real, 347-line, Product-approved design (2026-08-12) covering far more than "architecture" -- Plan/Subscription entities, fail-closed feature entitlements, payments (no gateway picked), billing history, AND a referral program. Proceed or hold? If proceeding, which slice?** | — | ✅ **Decided 2026-08-21.** Audited before asking, same discipline as every other D-row: confirmed via `grep` that zero `Plan`/`Subscription`/`Entitlement`/`Payment`/`Referral`/`Wallet` code exists anywhere in the backend (genuinely greenfield, matching the proposal's own Correction #1), that `FeatureFlag` (real, shipped, `V32__feature_flags.sql`) is a generic fail-open boolean toggle unrelated to entitlements and explicitly the WRONG default to copy for a paywall (proposal's own Correction #3), and that the proposal's own §2a prerequisite (reconcile `frontend/src/pages/landing/plans.ts` from Free/Premium/Family/Future to the approved Free/Plus/Premium taxonomy) **has already been completed** by a prior session -- `plans.ts` already shows the correct taxonomy with a comment citing the 2026-08-12 decision. Two forks put to the owner: **(1) timing** -- proceed now (read D-22's same-day scope note as the owner's own contemporaneous promotion out of the post-GA bucket) vs. hold PR4 with the rest of that bucket (AI Copilot, forecasting, referral system) until after v1.0 GA. **Owner chose proceed now.** **(2) scope** -- the recommended narrow slice (just the entitlement gate: `Plan`/`Subscription`/`FeatureEntitlement` schema + fail-closed `hasEntitlement(userId, featureKey)` + a `PremiumFeatureGate` frontend component, no payment gateway, no purchase/upgrade UI, no referral program) vs. a broader slice. **Owner chose broader** -- then, since "broader" bundled two materially different additions, asked a direct follow-up rather than guessing which: **owner chose BOTH billing-history scaffolding (schema + empty-state UI, no gateway) AND the full referral program** (codes, invite tracking, reward/credit via an append-only `wallet_ledger`, abuse detection reusing `RefreshToken`'s existing device/IP capture rather than building new fingerprinting, admin referral dashboard) -- a materially larger scope than D-22's own PR4 description named, on the owner's own explicit call. **Sequenced as three sub-PRs, same lettered-sub-PR discipline as PR3-A/B/C/D**: PR4-A (entitlement architecture -- the foundational piece the other two depend on), PR4-B (billing history scaffolding), PR4-C (referral program). **Still explicitly out of scope, per the proposal's own §5 and D-22's "do not ship priced UI"**: any specific price point, payment gateway selection, upgrade/downgrade timing, refund policy, trial terms -- all five remain open business/product decisions the schema is designed to support without presupposing. PR4-A merged 2026-08-21 ([PR #211](https://github.com/siddharth705/finora/pull/211)). PR4-B (billing history scaffolding) also merged 2026-08-21 ([PR #223](https://github.com/siddharth705/finora/pull/223)). PR4-C (referral program) also merged 2026-08-21 ([PR #232](https://github.com/siddharth705/finora/pull/232)) -- all three of D-28's sub-PRs now shipped. |
| **D-29** | **With §4a's Phase 1 (statement metadata reliability) closed — masked card/account number, total amount due, payment due date, deliberate Axis/Statement Date deferrals — what's the next parser-reliability phase?** | — | ✅ **Decided 2026-08-21: Phase 2 — Transaction Boundary & Extraction Reliability, starting with a 2A investigation, not implementation.** Owner's own framing: Phase 1 answered "can Finora correctly understand the statement's metadata," Phase 2 asks "can Finora correctly identify the actual transaction table, know when it starts and ends, and ensure every real transaction is imported without pulling in bank-generated noise." Scoped 2A (start detection) through 2H (trustworthy import-result reporting) — see new §4d for the full table. **2B (transaction end) flagged as the highest-value item**: the main extraction path can keep a section open past a closing marker on credit-card statements that continue with ads/rewards/T&Cs/interest examples after the last real transaction, so later content with a date and amount can be mistaken for another transaction. **Explicit instruction: do not fix 2B with a keyword list** — establish a real-corpus-evidenced boundary model first (closing headers, repeated table headers, page transitions, column geometry, or another structural signal), same "investigate before touching `groupIntoRows`" discipline Phase 1 already validated twice (the Payment Due Date ordinal-suffix fix and the Axis card-number root-cause correction both overturned an initially-plausible theory). **Not a v1.0 release gate**, same standing as Phase 1 (§4a) and the full-repo bug review (§4c) — parser-reliability work tracked separately from the launch critical path (§5). **Next task: Phase 2A — Transaction Boundary Investigation, no code changes until the evidence is in.** |
...
| Date | Change | Why |
|---|---|---|
| 2026-08-22 | **Re-baseline: 83% → 86%, 32 commits since `a73bee21` folded in (§1/§2/§4b/§4c/§7/§10 touched), on top of the same-day PR #247 fold-in of Phase 2A–2E.1 (§3/§4d, prior row).** Six threads closed: (1) **CI SPOF resolved** — PR #224 (`a1ff4bac`) moved every workflow off the self-hosted macOS runner onto GitHub-hosted `ubuntu-latest`, confirmed via `runs-on:` in all `.github/workflows/*.yml`; §7 R-6 closed, since a self-hosted runner executing fork-PR code under its registering user is a real exposure now that the repo is public — GitHub issues #101/#94 (self-hosted runner DB/container lifecycle) are likely stale/closeable as a result, flagged for the owner rather than closed here. (2) **§4c remediation**: 13 of 46 live findings closed across PR #227 (Bug 04/07/10 — the exact 3 Live Highs §10's QA Complete gate named as its blocker, now cleared), PR #229 (Bug 33/34/35), PR #234 (Bug 14/18/36/40/47), PR #238 (Bug 09/16 fixed, Bug 54 closed no-defect) — 33 live findings remain, unsequenced; PR #230 (open) covers 4 more (Bug 24/26/50/53). (3) **Security Sprint 3 merged** (PR #188, `5dcf8600`, SEC-05/10/11/12/16) — §4b's last open sprint. (4) **D-28 confirmed fully shipped**, all three sub-PRs merged (PR4-A #211, PR4-B #223, PR4-C #232) — already recorded in §11, verified consistent here. (5) **Rebrand + admin MFA complete**: admin-portal rebrand (#228) + dark mode/password toggle (#243) + MFA UI (#246, `483357da` — login challenge, self-service enrollment, recovery codes) all shipped; MFA enforcement stays behind `app.admin-mfa.enabled` (default `false`) as a deliberate activation gate, not remaining work. (6) Minor fixes folded without new line items: mobile Face ID AppState re-trigger (#220), `expo-screen-capture` iOS keyWindow patch (#219), native `confirm()` replaced with a custom dialog across frontend+admin-portal (#237), data export now covers soft-deleted goals+contributions (#217), Gmail merchant-template admin UI + 50 trusted domains seeded but disabled (#244/#235), lucide-react Instagram-icon fix (#241), typescript/sentry dependency bumps (#32/#36). **§2 weighted recompute, shown in full**: row 1 20%×95%=19.0, row 2 20%×85%=17.0, row 3 10%×92%=9.2, row 4 12%×97%=11.64, row 5 12%×93%=11.16, row 6 14%×64%=8.96, row 7 12%×76%=9.12 (held flat — none of this pass's fixes touch the store-enrolment gate). Sum 19.0+17.0+9.2+11.64+11.16+8.96+9.12 = **86.08 → 86%**, net **+3** over the 08-21 baseline's 82.9. **No date moved**: every gain this pass is engineering/QA/security velocity, not the store-enrolment/D-7/D-12 constraint that actually sets the date (§8, §9a) — consistent with this plan's own repeated finding that code velocity isn't what sets the date. **3 open PRs remain touching this area**: #230 (Phase 4c batch, in flight), #15 (lucide-react bump — likely now redundant, since #241 already hand-fixed lucide-react to 1.31.0 on `main`; flagged for the owner to close rather than closed here), #12 (jsdom bump, blocked on CI's Node 20→22 gap, tracked in issue #248). **Next highest-priority action, per §12 step 7**: the store-enrolment cluster (R-13, already flagged "top of the list") stays the single highest-priority item — it is the only Critical-severity, Certain-probability open risk in §7, and unlike PR #230 (incremental §4c cleanup) or issue #248 (unblocks one dependency PR), it is the actual gate on the launch date per §5a/§9a; Apple/Google enrolment and D-7/D-12 (pricing, privacy policy) remain untouched by this pass and should be the owner's next action | Direct execution of the owner's re-baseline request, applying §12's 7-step procedure in order against the 32 unfolded commits since `a73bee21`, on top of PR #247's same-day fold-in of the Phase 2 track |
| 2026-08-22 | **Phase 2 progress folded in: 2A–2D and 2E.1–2E.2 all complete and merged (§3/§4d touched, §2 percentages NOT recomputed -- Phase 2 stays outside the weighted score, same standing as §4a).** 2A (transaction-start investigation, PR #222), 2B (transaction-end closing-marker fix, PR #226), 2C (2 more banks' trailing-content triggers, PR #231), and 2D (row-completeness cross-check across all 21 ground-truthed real documents, 20/21 complete, PR #236) closed out the original D-29 scope table's first four rows. The owner then reframed row-correctness (2E) into its own investigate-then-design-then-implement sub-phase, posing the governing question directly: "for every transaction Finora imports, can we prove it came from the correct table, with the correct columns, and with complete narration?" 2E.1 (header-reconstruction investigation + design, PR #239) root-caused why a real header collapses when it's a genuine multi-column shape `mergeHeaderLines` has no model for on 3 real documents (SBI, IOB, HSBC), and produced a design corrected mid-review by the owner's own trace-level challenge (the reconstruction trigger has to be "is the ACCEPTED header weak," not "did a merge attempt fail" -- traced against the real code to show SBI's own accepted header was never itself the subject of a failed merge). 2E.2 (PR #245) shipped the first implementation -- deliberately held to SBI's exact single-fragment shape after the owner's explicit "hold this phase to SBI-only" call, made once real-corpus verification showed the general, multi-tier version of the same mechanism partially recovers a different real document (ICICI) but fails that document's own aggregate balance-total check. Three real bugs were found and fixed mid-build, before any of it reached a test: a preempted older recovery mechanism, a section-merge risk on SBI's own two cardholder sections, and two successive validation designs that each passed their own tests while missing a real data-corruption shape. 2E.3 (narration correctness) and 2E.4 (balance semantics) remain explicitly unverified hypotheses from the owner's own framing, not yet checked against evidence; 2E.5 (HSBC/IOB classification) and 2F–2H are not started | Direct record of six merged PRs' worth of Phase 2 progress that had no changelog entry since D-29 was decided on 08-21 -- same "close the reference gap the moment it's found" discipline this plan already applies to every other workstream, requested directly by the owner after PR #245 merged |
| 2026-08-21 | **Re-baseline: 81% → 83%, 52 merges since 08-17 folded in (§1/§2/§3/§4c/§4d/§11 all touched).** Requested directly by the owner after four days of unrecorded activity. Fresh evidence gathered, not carried forward: backend 3200/3200 tests (was 2833), frontend 569/admin 314/mobile 364 tests all green with real counts for the first time, `origin/main` tip re-baselined to `a73bee21`, R-18 (deployment failure) confirmed resolved via GitHub's deployment API. The headline % recompute is net **+2**, not a clean win — real shipped progress across 5 of 7 §2 rows (Google/Apple sign-in both platforms, PR3 A–D, security Sprints 1/2/4, mobile hardening + rebrand) was partly offset by a second, independent full-repository bug review (new §4c, 46 live findings including live Highs) that had merged to `main` four days ago with zero reference anywhere in this plan until now — the same "unreviewed surface" pattern this plan's own discipline already names. **Two other open PRs found touching this same file**: [#177](https://github.com/siddharth705/finora/pull/177) (titled "re-baseline to 83%," opened 08-17, never merged, and on inspection actually bundles an unrelated Personal CFO product-vision doc + a binary `.xlsx` file, not a re-baseline) and [#218](https://github.com/siddharth705/finora/pull/218) (a legitimate 1-line PR4-A changelog addition from a concurrent session, opened minutes before this pass started) — neither merged nor built upon here; flagged to the owner for a close/keep call rather than resolved unilaterally, per this plan's own parallel-session collision history (see the file's own header note) | Direct execution of the owner's explicit "run the re-baseline" instruction; the PR #177/#218 discovery is recorded rather than silently worked around, matching this plan's standing practice for every prior parallel-session collision |
| 2026-08-21 | **D-29 decided: Phase 2 — Transaction Boundary & Extraction Reliability scoped as the next parser-reliability phase, 2A (investigation) first.** Owner's own framing, given unprompted after reviewing Phase 1's completion: Phase 1 (§4a) answered whether Finora understands a statement's metadata; Phase 2 asks whether it correctly identifies the transaction table itself — start, end, and what's noise vs. a real row. 2B (transaction-end detection) named as highest-value: credit-card statements that continue past the last real transaction with ads/rewards/T&Cs can have their extraction section stay open, so trailing content with a date and amount risks being read as another transaction. Explicit instruction not to fix this with a keyword list — establish the real evidence model first, same discipline that caught two wrong initial theories during Phase 1 itself. See new §4d for the full 2A–2H scope table | Direct record of the owner's own next-phase decision, posed and decided in the same message — no engineering started yet, this is scoping only |
| 2026-08-21 | **Phase 1D closed: Statement Date investigated, deliberately not built** — [PR #215](https://github.com/siddharth705/finora/pull/215), a single-line addition to the Capability Backlog table in `financial-document-intelligence-principles.md` (not this file). Real-corpus evidence showed the extracted value would duplicate `statementPeriodEnd` on every credit-card statement checked; owner chose to skip the field rather than build something that tells the user what they already know. This closes out Phase 1's card/account-number and metadata scope — see D-29 immediately above for what comes next | Closing record for a deliberate scope decision, same as the Axis entry below — a "we investigated and chose not to build this" is as much a real outcome as a shipped feature and gets recorded the same way |
| 2026-08-21 | **PR4-C merged (Referral program) — [PR #232](https://github.com/siddharth705/finora/pull/232), squash `f0d42b9a`. Third and final of D-28's three sub-PRs -- Premium Layer now fully shipped.** Added `referral_codes`/`referrals`/an append-only `wallet_ledger` (V101): a shareable code generated lazily, redemption at self-service registration only (an optional, unvalidated `referralCode` -- an invalid or stale code is a silent no-op, never a blocked signup; deliberately not wired into `adminCreateUser`, which has no organic acquisition to track), and an automatic `REGISTERED` → `SUBSCRIBED` transition when a referred user's plan changes (still the only paid-plan path today, PR4-A). Reward crediting is admin-manual, not automatic -- the actual amount is still an open product decision (§10), same reasoning PR4-A's `changePlan` itself is admin-manual -- and fails closed on suspected self-referral, reusing `RefreshToken`'s existing device/IP capture rather than new fingerprinting (§4). User-facing "Refer & Earn" page and an admin Referrals dashboard, gated by `REFERRAL_MANAGEMENT_VIEW`/`_MANAGE` (V101), including the admin-portal `ADMIN_PORTAL_PERMISSIONS` registry entry -- the exact class of gap PR4-A's own self-review caught for `SUBSCRIPTION_MANAGEMENT_VIEW`. Threaded into `AccountPurgeSweepService` (both directions for `referrals`, since a purged user may appear as either referrer or referred). Self-review caught a real gap before ever pushing: `SubscriptionService.changePlan` reaches `ReferralService.onPlanChanged`, which writes an audit entry with no actor parameter -- exactly the bug shape `AuditActorAttributionTest` (FG-025) exists to catch, since this path is reachable from the admin subscription-change endpoint. Fixed by threading `actingAdminId` through before the fix was ever committed. Full backend suite 3248/3248 (every ArchUnit architecture guard included), frontend 583/583, admin portal 322/322, all 7 CI checks green | Direct execution of D-28's own decision (third and final of its three named sub-PRs), logged per §12's "every merge gets a changelog row" practice, same as PR4-A/PR4-B/PR3-A/B/C/D. The audit-attribution catch gets its own mention for the same reason every other pre-merge self-review finding in this plan does: found and fixed before merge, not silently folded in without a trace |
| 2026-08-21 | **PR4-B merged (Billing history scaffolding) — [PR #223](https://github.com/siddharth705/finora/pull/223), squash `9a14d81c`. Second of D-28's three sub-PRs.** Added `payments` (V100) -- schema only, no payment gateway selected yet (D-28/§10 still open) -- and a read-only `GET /api/v1/billing/history` over it, plus a user-facing "Billing History" page (account dropdown menu, next to Profile/Settings). Every user's history is genuinely empty today, correctly: nothing has ever inserted a payment row, so the empty state is the only state this feature can actually be in right now, not an unfinished branch -- the populated path is written and tested against real data (`BillingHistoryControllerIT`) so a future gateway integration only has to start inserting rows. Also closed a gap this sub-PR would otherwise have reintroduced: `AccountPurgeSweepService` now hard-deletes a user's `payments` on account deletion (ordered before the `subscriptions` delete, since a payment's `subscription_id` carries no `ON DELETE` behavior of its own), proven against a real Postgres the same way PR4-A's own subscription hard-delete was. Backend suite green (new + updated tests: `BillingHistoryServiceTest`, `BillingHistoryControllerIT`, `AccountPurgeSweepServiceTest`/`IT`, `EntitlementControllerIT` re-verified as a regression check), frontend suite 578/578 + `tsc -b` clean, all 7 CI checks green | Direct execution of D-28's own decision (second of its three named sub-PRs), logged per §12's "every merge gets a changelog row" practice, same as PR4-A/PR3-A/B/C/D. The purge-sweep gap is worth its own mention for the same reason PR4-A's own subscription gap was: a real, proactively-found correctness gap fixed pre-merge, not silently folded into the main diff without a trace |
| 2026-08-21 | **PR4-A merged (Premium Layer entitlement architecture) — [PR #211](https://github.com/siddharth705/finora/pull/211), squash `d0690a1c`.** First of D-28's three sub-PRs: `Plan`/`Subscription`/`FeatureEntitlement` schema plus a fail-closed `hasEntitlement(userId, featureKey)` check — deliberately the opposite default from the existing `FeatureFlag`, which fails open and was already named in D-28 as the wrong pattern to copy for a paywall. No payment gateway, no purchase/upgrade UI, no priced UI, per D-28's own explicit scope limit. PR4-B (billing-history scaffolding) and PR4-C (referral program) not started | Direct execution of D-28's already-decided scope; logged per this plan's "every merge gets a changelog row" practice |
| 2026-08-21 | **Phase 1C.2 closed: Axis's masked card number investigated, deferred as a genuine evidence gap** — [PR #212](https://github.com/siddharth705/finora/pull/212), a single-line addition to the same Capability Backlog table. Root cause found (the value exists in `auxiliaryText` but only adjacent to Axis-specific marketing boilerplate, not a genuine document-authored label/value relationship the way SBI's grid fallback has) after two initial root-cause theories were tried and found wrong — both corrections reported before any code was written. Owner's explicit call: document the structural reason, do not build a single-document-overfit heuristic | Same discipline as Phase 1D — a deferral with real evidence behind it is a real outcome |
| 2026-08-21 | **Phase 1C.1 closed: SBI grid-fallback card-number recovery** — [PR #208](https://github.com/siddharth705/finora/pull/208), after a stacked-PR recovery (the original #207 auto-closed when its base branch #201 was squash-merged and deleted; recovered by rebasing onto `main` and reopening against `main` directly). Extends `CARD_NUMBER_LABEL` recognition with a genuine multi-line grid fallback (`findGridValue`), lowers the digit floor 4→2 (SBI's real statement reveals only the last 2 digits), and explicitly does not add a mask-character requirement (a real HSBC account number in the corpus is genuinely unmasked, so requiring a mask would have broken it). Two real bugs caught and fixed via self-review before commit: an initial version of the grid fallback fired on any line matching the label anywhere, false-positiving on Axis/BOB/HSBC-savings — fixed with a `labelIsLastContentOnLine` gate; that gate then rejected a valid label-with-trailing-colon case, fixed with a colon-tolerant trim. 46 tests in `PdfMetadataExtractorTest`, all passing | Direct execution of the owner's approved 1C.1 scope, with both self-review corrections preserved in the record per the owner's own explicit request at approval time |
| 2026-08-21 | **Phase 1C merged: masked card/account number recognized for HDFC and Kotak** — [PR #201](https://github.com/siddharth705/finora/pull/201). New label vocabulary (Card Number/Card No./Credit Card No./Primary Card Number, alongside the existing Account Number), a loose masked-value shape tolerant of `X`/`x`/`*` masking characters, same-line and trailing-line matching. Scoped to HDFC + Kotak only per explicit instruction, not a general shape-only detector | Direct execution of approved 1C scope; logged per this plan's changelog practice |
| 2026-08-21 | **Phase 1B merged: credit-card Total Amount Due surfaced end-to-end** — [PR #198](https://github.com/siddharth705/finora/pull/198). Exposes the already-detected `CreditCardSummaryEvidence.totalAmountDue` through `DetectedAccountInfo`, the API, and a new frontend row labeled "Total amount due (detected)" — deliberately not just "Amount Due," to stay unambiguous against transaction amount / minimum due / outstanding balance, all of which already exist as separate concepts in the UI | Direct execution of approved 1B scope |
| 2026-08-21 | **Payment Due Date recovery fixed on ordinal-suffix dates and same-line label anchors** — [PR #197](https://github.com/siddharth705/finora/pull/197). The root cause was corrected mid-investigation: an initial doc-comment claim that the fix was "verified against a real credit-card statement's Payment Due Date field" was found stale after the actual root cause turned out to be different, and corrected across all 4 locations before this was reported as done | Same "verify before asserting provenance" discipline this plan already applies to every other finding |
| 2026-08-18/19 | **Evidence-based Import Reliability Status shipped** — [PR #182](https://github.com/siddharth705/finora/pull/182)/[#183](https://github.com/siddharth705/finora/pull/183)/[#184](https://github.com/siddharth705/finora/pull/184). A rule-based, non-weighted `ImportReliabilityStatus` (`CLEAN`/`REVIEW_RECOMMENDED`/`NEEDS_ATTENTION`) derived deterministically from evidence already in the pipeline (header-reconstruction findings, dropped-row reasons, finding outcomes, OCR provenance) — explicitly not a weighted score, matching `ImportVerifier`'s own standing doc-comment rejection of an invented weighting policy. Also surfaces OCR provenance (`TextSource`) to the API for the first time; it was previously discarded at the point of acquisition. Referenced in new §4d as the foundation Phase 2's "trustworthy import result" item (2H) should extend, not replace | Closes a gap a prior planning pass in this project had already scoped in detail — recorded here since the implementation is what actually matters, and because §4d cites it directly |
| 2026-08-21 | **Full-repository bug review discovered on `main`, not previously referenced anywhere in this plan — new §4c added.** [`BUG_REVIEW_REPORT.md`](../../quality/bug-reports/BUG_REVIEW_REPORT.md) (58 findings, 46 live including 3–4 Live High) merged via [PR #195](https://github.com/siddharth705/finora/pull/195) on 08-21, correcting the report's own tally. Separate review from the original 08-08 bug hunt (§4) — different scope, different numbering, not yet cross-checked for overlap. 3 Criticals were found and fixed during the review itself (an entirely unreachable merchant-learning queue, an unvalidated balance write, a merchant-collapse bug on UPI/NEFT rails) — real defects this plan had no prior record of. The 46 live findings have no owner sequencing decision yet, unlike §4b's security backlog | Same "close the reference gap the moment it's found" discipline §4a/§4b were each written to apply to their own workstreams |
| 2026-08-18–21 | **Smaller merges folded in without individual rows, batched here**: `4133910`-class infra fixes — testcontainers/plugin version drift ([#193](https://github.com/siddharth705/finora/pull/193)), `FilesystemStatementStorage` root-directory creation on construction ([#194](https://github.com/siddharth705/finora/pull/194)), learning-queue drain retry budget raised to survive full-suite backlog ([#199](https://github.com/siddharth705/finora/pull/199)), mobile typecheck fix after a dependency bump ([#190](https://github.com/siddharth705/finora/pull/190)), ADR-007 renumbered to ADR-009 to resolve a numbering collision ([#196](https://github.com/siddharth705/finora/pull/196)), a skip-to-content accessibility link on the landing page ([#206](https://github.com/siddharth705/finora/pull/206)), bundle identifiers renamed to `com.finoratech.app` on both mobile platforms ([#191](https://github.com/siddharth705/finora/pull/191)), and a `ProductIdentityResolver` audit that filed BH-061 as a new open finding ([#141](https://github.com/siddharth705/finora/pull/141), already reflected in §1/§4). Routine dependency-bump PRs (#161/#162/#163/#10/#202) intentionally excluded, per §5's own "batch them, they're not a workstream" note | Keeps the changelog proportionate — these are real merges but none individually changed a workstream's completion % or an owner decision |
| 2026-08-21 | **D-28 decided: PR4 (Premium Layer) proceeds now, scoped broader than D-22's own description.** Two real tensions surfaced and resolved before any code was written: a same-day-but-uncross-referenced conflict between D-22's PR4 approval and a separate post-GA-deferral decision (owner resolved in PR4's favor -- D-22 counts as the "explicit future decision" promoting it), and a scope gap between D-22's "architecture only" framing and the actual 347-line, Product-approved `billing-subscription-entitlements-proposal.md` design it points to. Owner chose the broader slice -- entitlement gate (`Plan`/`Subscription`/`FeatureEntitlement`, fail-closed `hasEntitlement`) **plus** billing-history scaffolding **plus** the full referral program, sequenced as PR4-A/B/C. No pricing, payment gateway, upgrade timing, refund policy, or trial terms decided -- all five stay open, unblocked by this. See §11's D-28 row for the full breakdown | Same audit-before-scoping discipline as every other D-row this session -- the §2a prerequisite (`plans.ts` taxonomy reconciliation) was checked and found already done by a prior session, and the scope conversation used a direct follow-up question rather than guessing which of two bundled options the owner meant |
| 2026-08-21 | **PR3-D merged (Activation funnel) — PR #200, squash `3be61a3`. Closes D-22's full 4-part PR3 sequence.** New admin-only "Activation funnel" panel on the Operational Dashboard: signup → first import → first budget → first goal, each a distinct-user count against the whole platform, reusing `PLATFORM_STATS_VIEW`. D-27 recorded and decided the same day: owner chose Admin Portal placement (matching 100% existing precedent) and a simple snapshot over a cohort/time-series (no scheduled pre-aggregation exists anywhere in this codebase yet). Each stage counts users who EVER reached it, not just currently-active -- same "persists indefinitely" precedent as `Goal.completedAt` (D-25 PR3-B) -- via native queries against `budgets`/`goals`/`statement_imports` deliberately bypassing `@SQLRestriction`. `signedUp` reuses the Operational Dashboard's own `totalUsers` figure so the two can't disagree. All 7 CI checks green, browser-verified live end-to-end against an isolated backend+DB (real signup, budget, and goal created via the actual API, funnel bars confirmed moving from 0/0/0/0 to real counts) | Direct execution of D-27's own decision, recorded per §12's "every merge gets a changelog row" practice, same as PR3-A/B/C |
...
| 2026-08-17 | **D-23 Phase 1 (web) merged** — [PR #160](https://github.com/siddharth705/finora/pull/160) squash-merged into `main` (`418c4825`) after a real merge conflict against `main` (which had advanced with PR3-A/PR3-B during CI's wait) was resolved and both full suites re-verified green against the merged code: backend 2975/2975, frontend 528/528, all 7 CI checks passed a second time post-merge. Supersedes the "pending CI/review" status in the entry below | Closes the loop the entry below opened — the PR referenced there is now actually on `main`, not just open |
| 2026-08-17 | **D-23 Phase 1 (web) built and PR opened** ([#160](https://github.com/siddharth705/finora/pull/160)), pending CI/review. Three self-review passes before this was ready, each catching a real bug: a null-returning `@Bean` method that broke the whole Spring context on startup; an account pre-hijacking vulnerability in the auto-link design (D-23's own account-linking fork didn't anticipate that password registration never verifies email ownership) — closed by adding real email verification for password registration and gating the auto-link on it, which is itself new, unscoped-at-decision-time engineering, not just a Google Sign-In implementation detail; a transaction-rollback bug in the fix for that (the same bug class this codebase's `login()`/`refresh()` already had to fix twice); and a bypass of the admin's registrations-enabled kill switch. Full detail in §11's D-23 row. Backend suite 2965/2965, frontend suite 500/500, live docker-compose + browser verification done. Phase 2 (mobile) not started | Same "record the moment it's found, not after the fact" discipline as every other security finding in this plan — an account-takeover-class bug found during self-review is exactly the kind of thing that needs to be visible here, not just fixed silently in the PR |
| 2026-08-17 | **PR3-B merged (Financial Journey backend) — PR #165, squash `e39485a`.** `GET /api/v1/dashboard/journey` now returns D-25's 5 milestones (`ACCOUNT_CREATED`/`FIRST_IMPORT`/`FIRST_BUDGET`/`FIRST_GOAL`/`FIRST_GOAL_ACHIEVED`), event-based real timestamps, no fixed Day-N framing. 4 milestones read straight off existing `User`/`StatementImport`/`Budget`/`Goal` timestamps; the 5th needed a new `Goal.completedAt` column (`V94__goal_completed_at.sql`), stamped once by `GoalService.markCompletedIfReached` the first time a goal's balance reaches its target (called from both `create()` and `addContribution()`), never overwritten afterward. Migration originally written as V93 — renumbered to V94 after checking open PRs and finding PR #160 (concurrent session, Google Sign-In) already claimed V93 for an unrelated migration, avoiding this session's own recurring Flyway-collision failure mode. All 7 CI checks green first run. Backend-only, per the owner's own PR3-A/B/C/D sequencing — frontend UI (PR3-C) and funnel analytics (PR3-D) remain separate, un-started | Direct execution of D-25's already-decided scope, not a new decision; logged per §12's "every merge gets a changelog row" practice |
| 2026-08-17 | **PR3-A merged (Financial Health Score minimum-data floor) — PR #159, squash `4614081`.** Below D-25's 10-transaction floor, `DashboardSummaryDto` now returns `healthScoreAvailable: false` plus the raw `healthScoreTransactionCount`/`healthScoreMinTransactions` pair instead of a misleadingly-computed score; `Dashboard.tsx` renders D-22's own "Getting Started" progress-card mockup in that state. `DashboardService.computeHealthScore`'s below-floor branch and the DTO/frontend/mobile-type plumbing were the only changes — `computeHealthScore`'s actual scoring logic is untouched once the floor is cleared. All 7 CI checks green first run, browser-verified live (below-floor and at-floor states) against an isolated throwaway backend+DB. This changelog entry was missed at merge time and is being recorded retroactively alongside PR3-B's entry above | Same retroactive-but-honest correction discipline this plan already applies elsewhere (see D-23→D-24 renumbering) rather than leaving a real merge silently undocumented |
| 2026-08-17 | **D-25 decided.** Owner answered all three scoping questions from the same-day proposal: health-score floor is a **transaction count** (e.g. ≥10), shown below the floor as a **"Getting Started / X% complete" progress card**; Financial Journey tracks **all 5 candidate milestones including goal-achieved** (clarified after an initially-ambiguous "create backend" answer -- confirmed explicitly rather than guessed, since it changes scope: goal-achieved needs a new `completedAt` column the other 4 milestones don't); framing is **event-based real elapsed days**, not the original pitch's fixed Day 1/3/15/30 schedule. No date change -- decision only, PR3 implementation not yet started | Same "clarify an ambiguous answer rather than guess" discipline as every other owner-decision row -- "create backend" alone didn't map to the proposal's own 4-vs-5-milestone framing, so a direct follow-up question resolved it before recording |
| 2026-08-17 | **D-25 recorded: PR3 (Real Dashboard Intelligence) scoped -- `dashboard-intelligence-pr3-proposal.md` added.** Of D-22's three named pieces, one (real cash-flow timeline) turns out to already be built -- Dashboard's Cash Flow Overview has had real per-month data since before D-22 was even written, dropped from PR3 entirely. The other two are real: the Financial Health Score minimum-data floor (traced `computeHealthScore` component-by-component, confirmed a thin-data user can land under 40 by construction) and the "Financial Journey" milestone section (confirmed no existing infrastructure app-wide; 3 of 4 candidate milestones' timestamps already exist unaggregated, the 4th -- goal completion -- needs new tracking). Two owner decisions posed rather than assumed: where the health-score floor sits (transaction count vs. time span) and what the exact milestone list is (plus whether to drop the original pitch's fixed "Day N" framing for an event-based one). No date change -- this is scoping, not new engineering; PR3 implementation has not started | Same audit-before-scoping discipline as every PR2-adjacent decision this session (D-22/D-23/D-24) -- catches a stale scope item before it becomes wasted implementation, same pattern D-19/D-20 hit repeatedly |
| 2026-08-16 | **D-23 recorded: real "Sign in with Google" approved, phased (web first, mobile reusing the same backend), three security/product forks decided by the owner rather than assumed.** Owner noticed the register screen has no Google option and asked why — investigated first and found it was never built, just anticipated (`AuthController`'s own `// TODO Phase 2: /oauth/google callback endpoint`, `GoogleOAuthProperties`'s own doc comment explicitly distinguishing itself from "Sign in with Google"). Presented three real forks rather than guessing, since the wrong call on account-linking specifically is a genuine account-takeover risk class, not just a UX preference: auto-link on Google's verified email (chosen) vs. reject; keep phone verification mandatory for Google signups (chosen) vs. exempt them; Google Identity Services button (chosen) vs. a popup+code-exchange flow matching Gmail sync's own shape. Scoping also found two real implementation simplifications: no schema migration needed, and the existing `PhoneChangeService` flow (built for PR #143, unrelated to this feature) is directly reusable for a Google signup's first phone number, cutting what looked like new-screen scope down to zero. See §11's D-23 row for the full breakdown. Phase 1 (backend + web) starts now | Same discipline as every other security/product fork this plan has surfaced explicitly rather than decided silently — account linking specifically gets this treatment because guessing wrong there is a security defect class, not a preference |
| 2026-08-16 | **D-24 decided (renumbered from a colliding D-23 -- a concurrent session independently used the same next-available ID for "Sign in with Google," now recorded above): owner chose (c), full page migration, for PR2's scope.** Of the three options `design-system-proposal.md` laid out, the owner picked the largest -- not just building the 7 components, but also closing the `EmptyState` gap on 6 other files and bringing `Reports.tsx`/`Investments.tsx` onto `MetricCard`/`ChartContainer`. No date change -- this is a scope decision, not new engineering; PR2 implementation itself has not started | Posed as an explicit three-way choice per this plan's own "no product decisions made for the owner" discipline, rather than defaulting to the narrowest slice the way most of this session's other D-rows recommended |
| 2026-08-16 | **PR #151 merged (Dashboard redesign + collapsible sidebar), D-22's next step (Design System, PR2) scoped -- `design-system-proposal.md` added.** All 7 CI checks green, squash-merged as `22e13b1a`. Scoping done before writing any component code, per §8a: grepped every app-shell page for each of the owner's 7 proposed components (`FinoraCard`, `MetricCard`, `EmptyState`, `SectionHeader`, `QuickActionCard`, `ChartContainer`, `PremiumBadge`) rather than assuming the pitch's "everything will drift" framing. Found real, counted drift on six of seven -- 43 raw duplications of the base card shell, two incompatible KPI-number type scales between Dashboard and Reports, 8 bare one-line empty-state sites across 6 files using 3 different color tokens (one, `Goals.tsx`, close to invisible), 5 copy-pasted section headers inside Dashboard alone, two independently hand-built Chart.js options objects -- and confirmed two (`QuickActionCard`, `PremiumBadge`) are pure prep-work with nothing to migrate yet. **D-24 recorded the same session** (originally logged as D-23, renumbered above): migration scope inside PR2 (components only vs. components + the one live small bug vs. full page migration) posed to the owner as an explicit three-way choice rather than decided here -- see §11 | Follows this plan's own established pattern (D-16, D-18, D-19) of auditing before scoping a cross-cutting effort, and keeps the migration-scope decision with the owner rather than assuming the largest or smallest option |
...
| 2026-08-16 | **D-21 Step 1's empty-state Dashboard redesigned from a screenshot reference, plus a new collapsible left sidebar — same session, not yet merged.** Owner shared a reference dashboard screenshot and asked to match it with a collapsible nav. Rebuilt `Dashboard.tsx`'s single welcome-gate (D-21 Step 1's original shape) into per-section empty states instead — the full 5-KPI/Cash-Flow/Spending/Accounts/Transactions/Budget/Goals shell stays visible, each empty section gets its own icon+CTA rather than one screen hiding all of it. Added a persisted collapsible `Sidebar.tsx` and a global "+ Add Transaction" button on `TopBar.tsx`, both reusing `AddTransactionModal.tsx` (extracted to a shared, self-fetching component so Dashboard and TopBar don't duplicate the accounts/categories fetch). Two screenshot elements deliberately not built and flagged rather than silently dropped: a "Finora Plus" premium upsell (D-7 pricing still unresolved) and a "This Month" date-range dropdown (would need new backend filtering; a non-functional dropdown is worse than none). Frontend 487/487 (`Sidebar.test.tsx`/`TopBar.test.tsx` new, 6 tests), `tsc --noEmit` clean, eslint clean, browser-verified live against an isolated throwaway backend+DB (registered via the real `/auth/register` endpoint, phone-verified by direct DB update since Firebase SMS OTP isn't reachable headlessly) — confirmed the redesigned shell, sidebar collapse/expand, and the TopBar modal's correct no-accounts gating all render as intended. **D-22 recorded the same session**: owner reviewed the redesign against product direction and set a 4-PR sequence (this redesign → Design System → Real Dashboard Intelligence → Premium Layer) — see §11 | Closes the loop D-21 Step 1 opened (empty dashboard was the known next activation problem) with the actual visual redesign the owner asked for, and records the next scope decision in the same session it was made, per §12's own rule |

> **Retained as a record, not as a current worklist. Read this note first.**
>
> Captured against `feat/merchant-learning-queue` at commit `5631bb9` (Deliverable 0), **before
> WI1 landed**. It is therefore stale by one commit in at least one important way, and findings
> must be re-checked against `main` before anyone acts on them.
>
> Verified stale: **Bug 01** ("the entire merchant-learning durable queue is unreachable — no
...
> consuming a retry attempt) was reviewed and kept as a deliberate trade, documented in
> `MerchantLearningEventWorker`.
>
> Everything else in here is unassessed. Treat each finding as a claim to verify, which is how the
> previous review's 38 findings were handled — 2 of those turned out to have accurate defects and
> wrong blast radii, and 1 prescribed a fix that would have caused a regression.
...
**Result:** 58 findings. Counts by severity:
...
## Description
The CSV text is handed to `new Blob([csv], { type: 'text/csv' })` with no `﻿` prefix and no charset in the MIME type. Excel on Windows decodes a BOM-less `.csv` using the system ANSI code page, not UTF-8.
...
## Title
`CurrentUser.id()` throws `IllegalStateException` on an anonymous request
...
## Title
`ApiResponse.ok(...)` reads MDC directly, so background threads emit a null `requestId`
...
## Title
`resolve` mixes fetched-by-id and fetched-by-user merchants, allowing a stale-alias repair to run per import row
...
## Description
When an alias row exists but the merchant it points at does not (a merchant deleted while its alias survived), the `orElseGet` silently creates a *new* merchant and calls `addAlias` — which immediately short-circuits at line 187 because the alias already exists and still points at the dead merchant.
...
- `.github/workflows/ci.yml`, `commitlint.config.js`, `.editorconfig`, `.vscode/`, `mobile/` scaffolding, `frontend/src/assets`, `admin-portal/src/assets`, `docs/` — inspected; nothing defective found.

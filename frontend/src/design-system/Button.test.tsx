...
// window.matchMedia) matches the convention Hero.test.tsx already uses -- framer-motion's real
// useReducedMotion reads `matchMedia('(prefers-reduced-motion)')`, not `(prefers-reduced-motion:
// reduce)`, so a matchMedia-based mock would need to get that exact string right; mocking the
// hook's return value tests this component's own gating logic without depending on it.
...
describe('Button', () => {
   ...
   * this the pending state reached assistive tech as nothing more than a disabled control. That is
   * a downgrade from the "Adding…"/"Saving…" label swaps this component replaced, which were real
   * readable text. The state now lives in the accessible name, plus aria-busy for AT that uses it.
   */

...
describe('Skeleton', () => {
  ...
  it('Row composes a title/subtitle line, a field block, and a button block', () => {
    ...
    // Row = 1 wrapper text block (title+subtitle) + a field block + a button block = 3 direct pulse-bearing descendants beyond the two text lines
  ...
  it('Region wraps its children with the loading-region accessibility contract', () => {
    const { getByRole, getByText } = render(
      <Skeleton.Region label="Loading transactions">
        <Skeleton.Row />
      </Skeleton.Region>
    );
  ...
  // The accessibility contract (aria-busy/role=status/aria-live) lives on Region, not on the
  // shapes themselves -- documenting that as a passing assertion, not just a doc comment, so a
  // future change that tries to "simplify" by dropping Region can't do it silently. A shape used
  // bare (no Region) is aria-hidden and announces nothing, which is why every per-page skeleton
  // composition in the roadmap is required to wrap its shapes in exactly one Region.

...
/**
 * One list row: title + subtitle line, a field-shaped block, a button-shaped block. Generalizes
 * ReviewCardSkeleton (PR #682) -- sized to match a typical AskOnceCard/MerchantGroupReviewCard/
 * Ledger table row so swapping in real content doesn't itself cause a layout shift.
 */
...
/** A KPI-tile/stat-card shape: icon circle, label bar, value bar. */
...
/**
 * One fixed-height chart placeholder, reused by every chart on Dashboard/Investments -- not a
 * bespoke "Cash Flow skeleton" / "Category skeleton" per chart (see the roadmap proposal's
 * reasoning for why that would otherwise happen).
 */
 ...
 * `Region` per logical loading area -- not one per shape, and not skipped.
 */

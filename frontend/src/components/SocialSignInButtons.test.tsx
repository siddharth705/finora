...
// file's job is narrower still: does the width this component learns from Google actually reach
// Apple's button, not the two buttons' own internal wiring (already covered in their own tests).
...
describe('SocialSignInButtons', () => {
  it("sizes Apple's button to Google's real rendered width once known", async () => {
    ...
    const { container } = render(
      <SocialSignInButtons googleText="signin_with" onGoogleCredential={vi.fn()} onAppleCredential={vi.fn()} onError={vi.fn()} />
    );
    await waitFor(() => expect(initialize).toHaveBeenCalled());

    // Apple's wrapper has no explicit width until Google reports one.
    const appleWrapper = container.querySelector('.mt-3') as HTMLElement;
    expect(appleWrapper.style.width).toBe('');

    fireResize(container.querySelector('[aria-busy]')!, 400);
    fireResize(container.querySelector('iframe')!, 420);

    await waitFor(() => expect(appleWrapper.style.width).toBe('420px'));
  });
});

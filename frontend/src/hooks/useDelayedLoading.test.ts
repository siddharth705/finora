...
describe('useDelayedLoading', () => {
  ...
  it('stays hidden for a fetch that resolves inside the showAfter window', () => {
    ...
    // Never crossed showAfter, so it should never have appeared, and shouldn't appear now either.
    act(() => {
  ...
  it('stays visible for at least minVisible even if the fetch finishes right after showing', () => {
    ...
    // Fetch finishes immediately after the skeleton appeared.
    rerender({ isLoading: false });
    act(() => {
      vi.advanceTimersByTime(100);
    });
    expect(result.current).toBe(true); // minVisible (300ms) hasn't elapsed yet

    act(() => {
      vi.advanceTimersByTime(250);
    });
    expect(result.current).toBe(false); // now it has
  });
  ...
  it('does not hide on a stale timer if loading restarts before the minVisible window closes', () => {
    ...
    rerender({ isLoading: true }); // loading again before the hide timer fires

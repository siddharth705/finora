 ...
 *
 * Copy lives in ./landing-config -- this file decides how it looks, not what it says.
 */
        ...
        <div className="grid lg:grid-cols-[minmax(0,1fr)_minmax(0,1.15fr)] gap-14 items-center">
          <Reveal>
            <h1 className="m-display mb-5">
              {hero.headline}
              <br />
              <span style={{ color: 'var(--m-brand)' }}>{hero.headlineAccent}</span>
            </h1>
            <p className="m-lead mb-8 max-w-lg">{hero.blurb}</p>
            <div className="flex flex-col sm:flex-row gap-3 mb-8">
              <Link to="/register" className="m-btn m-btn-primary w-full sm:w-auto">
                {hero.primaryCta} <ArrowRight size={16} />
              </Link>
              <a href="#how" className="m-btn m-btn-ghost w-full sm:w-auto">{hero.secondaryCta}</a>
            </div>

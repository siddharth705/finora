...
- **OTP phone verification** — `OtpService` issues a hashed, attempt-limited (5 tries), 10-minute-expiry 6-digit code via `SmsService`, mirroring the email pattern exactly: `TwilioSmsService` (real, activates when `TWILIO_ACCOUNT_SID`/`TWILIO_AUTH_TOKEN`/`TWILIO_FROM_NUMBER` are set) and `NoOpSmsService` (default — logs the code instead of sending, and exposes it as `devOtp` in the API response so the flow is still fully testable without a real Twilio account). New `VerifyPhone.tsx` page handles entry + resend. Registration automatically issues the first OTP; login/register both report `phoneVerified` so the frontend routes to `/verify-phone` only when needed.
- **A real bug caught while building this**: the OTP endpoints were initially going to live under `/api/v1/auth/**`, which is entirely `permitAll` in `SecurityConfig`. Since `sendPhoneOtp()`/`verifyPhoneOtp()` need a real authenticated user (`CurrentUser.id()`), putting them there would have let unauthenticated requests reach code that assumes a valid principal exists. Moved to a dedicated `PhoneController` at `/api/v1/phone/**` instead, which correctly falls under the default `authenticated()` rule.

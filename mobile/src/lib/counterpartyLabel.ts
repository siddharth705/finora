...
/**
 * Deliberate mirror of `frontend/src/lib/counterpartyLabel.ts`. There is no shared package between
 * these two apps -- `types/index.ts` is already duplicated the same way -- so the copies have to be
 * kept in step by hand, and the wording MUST match: the same transaction read on a phone and on the
 * web describing its counterparty two different ways would be its own small bug.
 *
